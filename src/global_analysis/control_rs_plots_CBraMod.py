...
def _setup_montage_and_sphere(n_channels, topos_mean=None):
    ...
    if n_channels == 256:
        ...
        print(f'  Warning: No standard montage for {n_channels} channels, creating generic layout')
...
def load_control_rs_data(orig_base_dir, recon_base_dir):
    ...
    for subject_dir in sorted(subject_dirs):
        ...
        # Build file paths
        orig_file = op.join(orig_base_dir, f"sub-{subject_id}", "ses-01", "orig", 
                           f"topos_sub-{subject_id}_ses-01.npz")
        recon_file = op.join(recon_base_dir, f"sub-{subject_id}", "ses-01", "recon",
                            f"topos_sub-{subject_id}_ses-01.npz")
        ...
        try:
            ...
            if topos_orig.shape != topos_recon.shape:
                print(f"   Skipping {subject_id}: shape mismatch orig={topos_orig.shape} vs recon={topos_recon.shape}")
                continue
    ...
    filtered_subjects = [s for s in subjects_data if s['n_channels'] == most_common_channels]
...
def create_control_rs_comparison_grid(output_dir, biomarker_filter=None):
    ...
    if biomarker_filter:
        ...
        available_biomarkers = [(m, MARKER_DISPLAY_NAMES.get(m, m)) for m in biomarker_filter if m in marker_names and m not in excluded_markers]
    else:
        # Use all available markers except excluded ones
        available_biomarkers = [(m, MARKER_DISPLAY_NAMES.get(m, m)) for m in marker_names if m not in excluded_markers]
    ...
    eps = 1e-10
    relative_diff = (topos_recon_mean - topos_orig_mean) / (np.abs(topos_orig_mean) + eps)
    
    # Set up montage
    info, sphere, outlines = _setup_montage_and_sphere(data['n_channels'], topos_orig_mean)
    
    # Create figure
    n_biomarkers = len(available_biomarkers)
    n_cols = 3  # Original, Reconstructed, Relative Difference
    
    fig, axes = plt.subplots(n_biomarkers, n_cols, figsize=(15, max(12, n_biomarkers * 2)))
    ...
    for row, (marker_name, display_name) in enumerate(available_biomarkers):
        ...
        # Add row label
        axes[row, 0].text(-0.25, 0.5, display_name, transform=axes[row, 0].transAxes,
        ...
        im_orig, _ = mne.viz.plot_topomap(
            orig_data, info, axes=axes[row, 0],
            vlim=(vmin_shared, vmax_shared), cmap='viridis',
            show=False, sphere=sphere, outlines=outlines,
            extrapolate='local', res=256, sensors=False, contours=0
        )
        axes[row, 0].set_title('')
        
        # Plot Reconstructed (column 1)
        im_recon, _ = mne.viz.plot_topomap(
            recon_data, info, axes=axes[row, 1],
            vlim=(vmin_shared, vmax_shared), cmap='viridis',
            show=False, sphere=sphere, outlines=outlines,
            extrapolate='local', res=256, sensors=False, contours=0
        )
        ...
        im_diff, _ = mne.viz.plot_topomap(
            diff_data, info, axes=axes[row, 2],
            vlim=(vmin_diff, vmax_diff), cmap='RdBu_r',
            show=False, sphere=sphere, outlines=outlines,
            extrapolate='local', res=256, sensors=False, contours=0
        )
        ...
        cbar_orig = plt.colorbar(im_orig, ax=axes[row, 1], shrink=0.7, aspect=15, pad=0.02)
        cbar_orig.ax.tick_params(labelsize=10)
        
        # Colorbar for Relative Difference
        cbar_diff = plt.colorbar(im_diff, ax=axes[row, 2], shrink=0.7, aspect=15, pad=0.02)
...
def create_control_rs_comparison_grid_selected(output_dir):
    ...
    return create_control_rs_comparison_grid(output_dir, biomarker_filter=selected_biomarkers)
...
def load_control_rs_data_three_sources(orig_base_dir, cbramod_base_dir, totem_base_dir):
    ...
    for base_dir, name in [(orig_base_dir, "Original"), (cbramod_base_dir, "CBraMod"), (totem_base_dir, "TOTEM")]:
    ...
    for subject_dir in sorted(subject_dirs):
        ...
        orig_file = op.join(orig_base_dir, f"sub-{subject_id}", "ses-01", "orig", 
                           f"topos_sub-{subject_id}_ses-01.npz")
        cbramod_file = op.join(cbramod_base_dir, f"sub-{subject_id}", "ses-01", "recon",
                              f"topos_sub-{subject_id}_ses-01.npz")
        totem_file = op.join(totem_base_dir, f"sub-{subject_id}", "ses-01", "recon",
                            f"topos_sub-{subject_id}_ses-01.npz")
        ...
        try:
            ...
            common_markers = sorted(set(orig_markers) & set(cbramod_markers) & set(totem_markers))
...
def create_control_rs_three_way_comparison_grid(output_dir, biomarker_filter=None):
    ...
    # Load data
    data = load_control_rs_data_three_sources(orig_base_dir, cbramod_base_dir, totem_base_dir)
    ...
    # Compute mean across subjects
    topos_orig_mean = np.mean(data['topos_orig'], axis=0)  # (n_markers, n_channels)
    topos_cbramod_mean = np.mean(data['topos_cbramod'], axis=0)  # (n_markers, n_channels)
    ...
    for row, (marker_name, display_name) in enumerate(available_biomarkers):
        ...
        vmin_shared = min(np.nanmin(orig_data), np.nanmin(cbramod_data), np.nanmin(totem_data))
        vmax_shared = max(np.nanmax(orig_data), np.nanmax(cbramod_data), np.nanmax(totem_data))
        ...
        im_cbramod, _ = mne.viz.plot_topomap(
            cbramod_data, info, axes=axes[row, 2],
            vlim=(vmin_shared, vmax_shared), cmap='viridis',
            show=False, sphere=sphere, outlines=outlines,
            extrapolate='local', res=256, sensors=False, contours=0
        )
        ...
        im_totem, _ = mne.viz.plot_topomap(
            totem_data, info, axes=axes[row, 1],
            vlim=(vmin_shared, vmax_shared), cmap='viridis',
            show=False, sphere=sphere, outlines=outlines,
            extrapolate='local', res=256, sensors=False, contours=0
        )
...
def create_control_rs_three_way_comparison_grid_selected(output_dir):
    ...
    return create_control_rs_three_way_comparison_grid(output_dir, biomarker_filter=selected_biomarkers)


def load_control_rs_data_four_sources(orig_base_dir, cbramod_base_dir, totem_base_dir, labram_base_dir):
    ...
    for base_dir, name in [(orig_base_dir, "Original"), (cbramod_base_dir, "CBraMod"), 
                           (totem_base_dir, "TOTEM"), (labram_base_dir, "LaBram")]:
        if not op.exists(base_dir):
            print(f"   Error: {name} directory not found: {base_dir}")
            return None
    ...
    for subject_dir in sorted(subject_dirs):
        ...
        totem_file = op.join(totem_base_dir, f"sub-{subject_id}", "ses-01", "recon",
                            f"topos_sub-{subject_id}_ses-01.npz")
        labram_file = op.join(labram_base_dir, f"sub-{subject_id}", "ses-01", "recon",
                             f"topos_sub-{subject_id}_ses-01.npz")
        ...
        try:
            ...
            common_markers = sorted(set(orig_markers) & set(cbramod_markers) & set(totem_markers) & set(labram_markers))
            ...
            # Validate shapes
            if not (topos_orig.shape == topos_cbramod.shape == topos_totem.shape == topos_labram.shape):
                print(f"   Skipping {subject_id}: shape mismatch")
                continue
...
def create_control_rs_four_way_comparison_grid(output_dir, biomarker_filter=None):
    ...
    # Load data
    data = load_control_rs_data_four_sources(orig_base_dir, cbramod_base_dir, totem_base_dir, labram_base_dir)
    ...
    fig, axes = plt.subplots(n_biomarkers, n_cols, figsize=(20, max(12, n_biomarkers * 2)))
    ...
    # Add column titles
    col_titles = ['Original',  'Reconstructed (TOTEM)','Reconstructed (CBraMod)', 'Reconstructed (LaBram)']
    for col, title in enumerate(col_titles):
        axes[0, col].text(0.5, 1.15, title, transform=axes[0, col].transAxes,
    ...
    for row, (marker_name, display_name) in enumerate(available_biomarkers):
        ...
        # Add row label
        axes[row, 0].text(-0.25, 0.5, display_name, transform=axes[row, 0].transAxes,
                         ha='right', va='center', fontsize=22, rotation=0)
        ...
        vmin_shared = min(np.nanmin(orig_data), np.nanmin(cbramod_data), np.nanmin(totem_data), np.nanmin(labram_data))
        vmax_shared = max(np.nanmax(orig_data), np.nanmax(cbramod_data), np.nanmax(totem_data), np.nanmax(labram_data))
        ...
        im_labram, _ = mne.viz.plot_topomap(
            labram_data, info, axes=axes[row, 3],
            vlim=(vmin_shared, vmax_shared), cmap='viridis',
            show=False, sphere=sphere, outlines=outlines,
            extrapolate='local', res=256, sensors=False, contours=0
        )
...
def create_control_rs_four_way_comparison_grid_selected(output_dir):
    ...
    return create_control_rs_four_way_comparison_grid(output_dir, biomarker_filter=selected_biomarkers)
...
def main():
    ...
    if args.all:
        ...
        if args.selected_only:
            output_path = create_control_rs_four_way_comparison_grid_selected(args.output_dir)
        else:
        ...
        if args.selected_only:
            output_path = create_control_rs_three_way_comparison_grid_selected(args.output_dir)
        else:
