...
try:
    ...
    HAS_SKIMAGE = False
    print(
        "Warning: scikit-image not available. SSIM computation will use custom implementation."
    )
...
def _setup_montage_and_sphere(n_channels, topos_mean=None):
    ...
    if n_channels == 256:
        ...
        sphere = "auto"
        outlines = "head"
        print("  Created standard montage for 128 channels")
        ...
        sphere = "auto"
        outlines = "head"
        print("  Created standard montage for 64 channels")
...
class StatisticalAnalyzer:
    ...
    def __init__(self, results_dir, output_dir, patient_labels_file, data_dir=None):
        ...
        self.marker_names = [
            name
            for idx, name in enumerate(all_marker_names)
            if idx not in self.excluded_marker_indices
        ]
        self.marker_short_names = [
            name
            for idx, name in enumerate(all_marker_short_names)
            if idx not in self.excluded_marker_indices
        ]
        ...
        for idx in self.excluded_marker_indices:
            print(
                f"  - Excluded: {all_marker_names[idx]} ({all_marker_short_names[idx]})"
            )
        print(
            f"  Analyzing {len(self.marker_names)} markers (down from {len(all_marker_names)})\n"
        )
    ...
    def _load_patient_labels(self):
        ...
        if not op.exists(self.patient_labels_file):
            raise FileNotFoundError(
                f"Patient labels file not found: {self.patient_labels_file}"
            )
    ...
    def load_topographic_data(self):
        ...
        for subject_dir in subject_dirs:
            ...
            for session_dir in session_dirs:
                ...
                topo_orig_file = op.join(
                    orig_dir, f"topod_{subject_id}_ses-{session_id}_orig.npz"
                )
        ...
        if all_shapes:
            ...
            print(
                f"  Expected shape: {expected_shape} (found in {shape_counts[expected_shape]} samples)"
            )
        ...
        for subject_dir in subject_dirs:
            ...
            for session_dir in session_dirs:
                ...
                topo_recon_file = op.join(
                    recon_dir, f"topod_{subject_id}_ses-{session_id}_recon.npz"
                )
                    ...
                    try:
                        ...
                        def extract_array(npz_data, expected_type):
                            ...
                            if isinstance(npz_data, dict):
                                ...
                                return npz_data
                            raise ValueError(
                                f"Could not extract array from {expected_type} NPZ data"
                            )
                        ...
                        if (
                            ...
                            and topos_orig.shape != expected_shape
                        ):
                            print(
                                f"  ⚠️  Skipped {subject_id}: shape {topos_orig.shape} != expected {expected_shape}"
                            )
        ...
        if not topos_orig_list:
            raise ValueError(
                "No valid topographic data could be loaded from the new NPZ structure"
            )
        ...
        keep_indices = [
            idx for idx in all_marker_indices if idx not in self.excluded_marker_indices
        ]
        ...
        print(
            f"  Removed {len(self.excluded_marker_indices)} non-normalized PSD markers\n"
        )
    ...
    def compute_ssmi(self, topos_orig, topos_recon):
        ...
        print(
            f"  Mean SSMI per marker: {np.mean(ssmi_per_marker):.6f} ± {np.std(ssmi_per_marker):.6f}"
        )

        return results

    def perform_wilcoxon_tests(
        self, topos_orig, topos_recon, subject_ids=None, group_name="all_subs"
    ):
        """
        ...
        n_subjects, n_markers, n_channels = topos_orig.shape
        print(
            f"  Data shape: {n_subjects} subjects, {n_markers} markers, {n_channels} channels"
        )
        print(
            f"  Total tests: {n_markers * n_channels} ({n_markers} markers × {n_channels} channels)"
        )
        ...
        for marker_idx in range(n_markers):
            print(
                f"  Testing marker {marker_idx + 1}/{n_markers} ({self.marker_short_names[marker_idx] if marker_idx < len(self.marker_short_names) else f'M{marker_idx}'})...",
                end="\r",
            )
                ...
                if len(orig_valid) < 5:  # Need minimum samples for reliable test
                    ...
                    try:
                        ...
                        statistic, p_value = wilcoxon(
                            orig_valid, recon_valid, alternative="two-sided"
                        )
            ...
            marker_results = {
                ...
                "marker_name": self.marker_names[marker_idx]
                if marker_idx < len(self.marker_names)
                else f"M{marker_idx}",
                "marker_short_name": self.marker_short_names[marker_idx]
                if marker_idx < len(self.marker_short_names)
                else f"M{marker_idx}",
                ...
                "min_p_value": float(np.nanmin(p_values))
                if len(valid_p_values) > 0
                else np.nan,
                "max_p_value": float(np.nanmax(p_values))
                if len(valid_p_values) > 0
                else np.nan,
                ...
                "proportion_channels_significant_05": float(
                    np.sum(p_values < 0.05) / np.sum(np.isfinite(p_values))
                )
                if np.sum(np.isfinite(p_values)) > 0
                else 0.0,
            }
        ...
        results["summary"] = {
            "mean_p_value_across_all": float(np.mean(all_mean_p_values))
            if all_mean_p_values
            else np.nan,
            "median_p_value_across_all": float(np.median(all_mean_p_values))
            if all_mean_p_values
            else np.nan,
        ...
        print(
            f"  Summary: {total_sig_05}/{n_markers * n_channels} channel tests significant at p<0.05 "
            + f"({results['summary']['proportion_significant_05']:.1%})"
        )

        return results

    def perform_ttest_tests(
        self, topos_orig, topos_recon, subject_ids=None, group_name="all_subs"
    ):
        """
    ...
    def compute_structural_similarity(
        ...
        for subj_idx in range(n_subjects):
            ...
            print(
                f"  Processing subject {subj_idx + 1}/{n_subjects}: {subject_id}...",
                end="\r",
            )
        ...
        print(
            f"  Overall mean SSIM: {results['overall_stats'].get('overall_mean_ssim', 'N/A'):.4f}"
        )
    ...
    def plot_wilcoxon_results(
        self, wilcoxon_results, topos_orig, group_name="all_subs"
    ):
        """
    ...
    def plot_wilcoxon_topomaps(
        ...
        cbar.ax.axhline(
            y=-np.log10(0.05), color="orange", linestyle="--", linewidth=2, alpha=0.7
        )
        cbar.ax.axhline(
            y=-np.log10(0.01), color="red", linestyle="--", linewidth=2, alpha=0.7
        )
        cbar.ax.text(
            3.5, -np.log10(0.05), "p=0.05", fontsize=8, color="orange", weight="bold"
        )
        cbar.ax.text(
            3.5, -np.log10(0.01), "p=0.01", fontsize=8, color="red", weight="bold"
        )

        plt.suptitle(
            f"Topographic Distribution of p-values (Wilcoxon Test)\n"
            f"Group: {group_name.upper()} | n={wilcoxon_results['n_subjects']} subjects",
            fontsize=14,
            weight="bold",
            y=0.98,
    ...
    def plot_ttest_results(self, ttest_results, topos_orig, group_name="all_subs"):
        ...
        ax1.set_ylabel("Mean p-value across channels", fontsize=11, weight="bold")
        ax1.set_title(
            f"Channel-wise Paired T-Test: Mean p-value per Marker\n"
    ...
    def plot_ttest_topomaps(self, ttest_results, topos_orig, group_name="all_subs"):
        ...
        plt.suptitle(
            f"Topographic Distribution of p-values (Paired T-Test)\n"
            f"Group: {group_name.upper()} | n={ttest_results['n_subjects']} subjects",
            fontsize=14,
            weight="bold",
            y=0.98,
    ...
    def plot_std_topomaps(self, topos_orig, topos_recon, group_name="all_subs"):
        ...
        for marker_idx in range(n_markers):
            marker_name = (
                self.marker_short_names[marker_idx]
                if marker_idx < len(self.marker_short_names)
                else f"M{marker_idx}"
            )
        ...
        plt.suptitle(
            f"Standard Deviation of Differences (Original - Reconstructed)\n"
    ...
    def plot_mean_diff_topomaps(self, topos_orig, topos_recon, group_name="all_subs"):
        ...
        plt.suptitle(
            f"Mean Difference Across Subjects (Original - Reconstructed)\n"
            f"Group: {group_name.upper()} | n={n_subjects} subjects",
            fontsize=14,
            weight="bold",
            y=0.98,
    ...
    def plot_std_topomaps_scaled(self, topos_orig, topos_recon, group_name="all_subs"):
        ...
        plt.suptitle(
            f"Standard Deviation of Differences (Scaled to Original/Reconstructed Range)\n"
            f"Group: {group_name.upper()} | n={n_subjects} subjects",
            fontsize=14,
            weight="bold",
            y=0.98,
    ...
    def plot_mean_diff_topomaps_scaled(
        self, topos_orig, topos_recon, group_name="all_subs"
    ):
        """
        Create topographic maps showing mean difference (original - reconstructed) across subjects,
        with colorbar scale based on original/reconstructed marker ranges.
        ...
        if not HAS_MNE:
            print(
                "  ⚠️  MNE not available, skipping scaled mean difference topographic plots"
            )
            return
        ...
        plt.suptitle(
            f"Mean Difference Across Subjects (Scaled to Original/Reconstructed Range)\n"
            f"Group: {group_name.upper()} | n={n_subjects} subjects",
            fontsize=14,
            weight="bold",
            y=0.98,
        ...
        # Save plot
        plot_path = op.join(
            self.plots_dir, f"mean_diff_topomaps_scaled_{group_name}.png"
        )
        plt.savefig(plot_path, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_ssim_results(self, ssim_results, group_name="all_subs"):
        ...
        ax.set_title(
            f"Structural Similarity: Original vs Reconstructed Topographic Markers\n"
        ...
        # Add reference lines
        ax.axhline(
            y=1.0, color="green", linestyle="--", alpha=0.7, label="Perfect similarity"
        )
        ax.axhline(
            y=0.8, color="orange", linestyle="--", alpha=0.7, label="Good similarity"
        )
        ax.axhline(
            y=0.6, color="red", linestyle="--", alpha=0.7, label="Moderate similarity"
        )
    ...
    def plot_ssmi_results(self, ssmi_results):
        ...
        ax1.bar(
            marker_labels, ssmi_results["ssmi_per_marker"], alpha=0.7, color="coral"
        )
        ax1.axhline(
            y=ssmi_results["mean_ssmi_per_marker"],
            color="red",
            linestyle="--",
        ...
        ax2.bar(
            channel_indices,
            ssmi_results["ssmi_per_channel"],
            alpha=0.7,
            color="mediumseagreen",
        )
        ax2.axhline(
            y=ssmi_results["mean_ssmi_per_channel"],
            color="green",
            linestyle="--",
    ...
    def analyze_group(
        self, topos_orig, topos_recon, subject_ids, group_name="all_subs"
    ):
        """
        ...
        wilcoxon_results = self.perform_wilcoxon_tests(
            topos_orig, topos_recon, subject_ids, group_name
        )
        ttest_results = self.perform_ttest_tests(
            topos_orig, topos_recon, subject_ids, group_name
        )
        ssim_results = self.compute_structural_similarity(
            topos_orig, topos_recon, subject_ids, group_name
        )
        ...
        json_path = op.join(
            self.results_data_dir, f"statistical_results_{group_name}.json"
        )
    ...
    def run_analysis(self, analyze_by_groups=True):
        ...
        all_results["all_subs"] = self.analyze_group(
            topos_orig, topos_recon, subject_ids, "all_subs"
        )

        # Analyze by groups if requested and patient labels are available
        if (
            analyze_by_groups
            and hasattr(self, "patient_labels")
            and self.patient_labels is not None
        ):
            ...
            for subj_id in subject_ids:
                ...
                for possible_id in possible_ids:
                    if possible_id in patient_df["subject"].values:
                        ...
                        group = row.get(
                            "state", row.get("diagnostic_crs_final", "Unknown")
                        )
            ...
            for group_name, group_subjects in groups.items():
                if len(group_subjects) >= 3:  # Minimum subjects for meaningful analysis
                    ...
                    print(
                        f"⏭️  Skipping group '{group_name}' - insufficient subjects ({len(group_subjects)} < 3)"
                    )

        # Save combined results
        combined_json_path = op.join(
            self.results_data_dir, "all_statistical_results.json"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Statistical analysis across multiple subjects"
    )
    parser.add_argument(
        "--results-dir", required=True, help="Path to results directory"
    )
    parser.add_argument(
        "--output-dir", required=True, help="Output directory for statistical results"
    )
    parser.add_argument(
        "--patient-labels", required=True, help="Path to patient labels CSV file"
    )
