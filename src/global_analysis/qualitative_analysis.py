...
def load_epoch_data(
    subject_id: str, session: str, session_path: str, epoch_type: str = "original"
) -> mne.Epochs:
    """
...
def load_subject_session_data(
    subject_id: str, session: str, session_path: str
...
class QualitativeAnalysis:
    ...
    def run_analysis(
        ...
        only_correlation_grid: bool = False,
    ):
        """
        ...
        if subjects is not None:
            ...
            selected = random.sample(
                subjects_sessions, min(num_subjects, len(subjects_sessions))
            )
        ...
        for item in selected:
            ...
            try:
                ...
                print(
                    f"  Original epochs: {len(original_epochs)} epochs, {original_epochs.info['nchan']} channels"
                )
                print(
                    f"  Reconstructed epochs: {len(recon_epochs)} epochs, {recon_epochs.info['nchan']} channels"
                )

                # Run analyses
                self.first_second_momentum(
                    original_epochs, recon_epochs, subject_id, session
                )
                self.mean_of_difference(
                    original_epochs, recon_epochs, subject_id, session
                )
                self.time_frequency_decomposition(
                    original_epochs, recon_epochs, subject_id, session
                )
                ...
                print(
                    f"  Error processing subject {subject_id}, session {session}: {e}"
                )
                continue
    ...
    def first_second_momentum(
        self,
        original_epochs: mne.Epochs,
        recon_epochs: mne.Epochs,
        subject_id: str,
        session: str,
    ):
        """
        ...
        axes[2, 0].set_title(
            f"Diff of Mean (Recon - Orig) - sub-{subject_id}_ses-{session}"
        )
        ...
        axes[2, 1].set_title(
            f"Diff of Std (Recon - Orig) - sub-{subject_id}_ses-{session}"
        )
        ...
        output_file = op.join(
            self.output_dir,
            f"sub-{subject_id}",
            f"momentum_sub-{subject_id}_ses-{session}.png",
        )
        ...
        axes[0, 0].set_title(
            f"Original Mean (Time × Epochs) - sub-{subject_id}_ses-{session}"
        )
        ...
        axes[0, 1].set_title(
            f"Original Std (Time × Epochs) - sub-{subject_id}_ses-{session}"
        )
        ...
        axes[1, 0].set_title(
            f"Reconstructed Mean (Time × Epochs) - sub-{subject_id}_ses-{session}"
        )
        ...
        axes[1, 1].set_title(
            f"Reconstructed Std (Time × Epochs) - sub-{subject_id}_ses-{session}"
        )
        ...
        output_file = op.join(
            self.output_dir,
            f"sub-{subject_id}",
            f"time_epochs_sub-{subject_id}_ses-{session}.png",
        )
        plt.savefig(output_file, dpi=150, bbox_inches="tight")
        plt.close()
        ...
        axes[0, 0].set_title(
            f"Original Mean (Time × Channels) - sub-{subject_id}_ses-{session}"
        )
        axes[0, 0].set_xlabel("Time")
        axes[0, 0].set_ylabel("Channels")
        ...
        axes[0, 1].set_title(
            f"Original Std (Time × Channels) - sub-{subject_id}_ses-{session}"
        )
        axes[0, 1].set_xlabel("Time")
        axes[0, 1].set_ylabel("Channels")
        ...
        axes[1, 0].set_title(
            f"Reconstructed Mean (Time × Channels) - sub-{subject_id}_ses-{session}"
        )
        axes[1, 0].set_xlabel("Time")
        axes[1, 0].set_ylabel("Channels")
        ...
        axes[1, 1].set_title(
            f"Reconstructed Std (Time × Channels) - sub-{subject_id}_ses-{session}"
        )
        axes[1, 1].set_xlabel("Time")
        axes[1, 1].set_ylabel("Channels")
        ...
        axes[2, 0].set_title(
            f"Diff of Mean (Recon - Orig) (Time × Channels) - sub-{subject_id}_ses-{session}"
        )
        ...
        axes[2, 1].set_title(
            f"Diff of Std (Recon - Orig) (Time × Channels) - sub-{subject_id}_ses-{session}"
        )
        ...
        output_file = op.join(
            self.output_dir,
            f"sub-{subject_id}",
            f"time_channels_sub-{subject_id}_ses-{session}.png",
        )
        plt.savefig(output_file, dpi=150, bbox_inches="tight")
        plt.close()
        ...
        output_file = op.join(
            self.output_dir,
            f"sub-{subject_id}",
            f"time_channels_shared_cmap_sub-{subject_id}_ses-{session}.png",
        )
        plt.savefig(output_file, dpi=150, bbox_inches="tight")
        plt.close()
        ...
        orig_mean_across_channels = np.mean(
            orig_time_channel_mean, axis=0
        )  # Shape: (times,)
        recon_mean_across_channels = np.mean(
            recon_time_channel_mean, axis=0
        )  # Shape: (times,)
        ...
        ax.set_title(
            f"Pearson Correlation between Original and Reconstructed - sub-{subject_id}_ses-{session}"
        )
        ...
        output_file = op.join(
            self.output_dir,
            f"sub-{subject_id}",
            f"correlation_mi_sub-{subject_id}_ses-{session}.png",
        )
        plt.savefig(output_file, dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def mean_of_difference(
        ...
        ax.set_title(
            f"Mean of Difference (Recon - Orig) across Epochs\nsub-{subject_id}_ses-{session}"
        )
        ax.set_xlabel("Time")
        ...
        output_file = op.join(
            self.output_dir,
            f"sub-{subject_id}",
            f"mean_of_difference_sub-{subject_id}_ses-{session}.png",
        )
        plt.savefig(output_file, dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def time_frequency_decomposition(
        ...
        output_file = op.join(
            self.output_dir,
            f"sub-{subject_id}",
            f"psd_sub-{subject_id}_ses-{session}.png",
        )
        plt.savefig(output_file, dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def pixel_corr(
        ...
        output_file = op.join(
            self.output_dir,
            f"sub-{subject_id}",
            f"corr_sub-{subject_id}_ses-{session}.png",
        )
        plt.savefig(output_file, dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def correlation_heatmaps_grid(self, fif_dir: str):
        ...
        if len(subjects_sessions) < 9:
            print(
                f"Warning: Only {len(subjects_sessions)} subject-sessions found, using all available."
            )
            selected = subjects_sessions
        else:
        ...
        # Save the figure
        output_file = op.join(
            self.output_dir, "correlation_heatmaps_grid_9subjects.png"
        )
        plt.savefig(output_file, dpi=150, bbox_inches="tight")
        plt.close()
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="/data/project/eeg_foundation/src/doc_benchmark/results/new_results/GLOBAL/qualitative",
        help="Directory to save analysis results",
    )
    parser.add_argument(
        "--only-correlation-grid",
        action="store_true",
