...
try:
    ...
    HAS_SKLEARN = False
    print(
        "Warning: sklearn not available. Mutual information computation will be skipped."
    )
...
def _setup_montage_and_sphere(n_channels, topos_mean=None):
    ...
    if n_channels == 256:
        ...
        print(
            f"  ⚠️  No standard montage for {n_channels} channels, creating generic layout"
        )
...
def load_patient_labels(patient_labels_file):
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            state = row[
                "diagnostic_crs_final"
            ]  # Use diagnostic_crs_final instead of state
...
def load_group_topo_data(
    ...
    for subject_dir in sorted(subject_dirs):
        ...
        for session_dir in sorted(session_dirs):
            ...
            try:
                ...
                topos_orig_file = op.join(
                    orig_dir, f"topos_sub-{subject_id}_{session_dir}.npz"
                )
                topos_recon_file = op.join(
                    recon_dir, f"topos_sub-{subject_id}_{session_dir}.npz"
                )
                ...
                topos_orig, marker_names = extract_topo_markers(
                    np.load(topos_orig_file)
                )
    ...
    if allow_heterogeneous:
        ...
        print(
            f"   ✓ Heterogeneous mode: keeping all {len(subjects_data)} subjects with varying shapes"
        )
        return subjects_data, subject_ids, None
    else:
        # For other groups: validate shape consistency across subjects
        print("   🔍 Validating shape consistency across subjects...")
        expected_shape = None
        valid_subjects_data = []
        ...
        for i, subject_data in enumerate(subjects_data):
            ...
            if expected_shape is None:
                ...
                print(
                    f"      ⚠️  Skipping subject {subject_data['subject_id']}: shape mismatch {current_shape} vs expected {expected_shape}"
                )
                continue
        ...
        if len(valid_subjects_data) == 0:
            print(
                f"   ❌ No subjects with consistent data shapes found for {group_name}"
            )
            return None, None, None

        print(
            f"   ✓ {len(valid_subjects_data)} subjects with consistent shapes retained"
        )

        # Convert to numpy arrays
        topos_orig_all = np.array([s["topos_original"] for s in valid_subjects_data])
        topos_recon_all = np.array(
            [s["topos_reconstructed"] for s in valid_subjects_data]
        )
...
def get_marker_data_for_group(group_data, marker_name):
    ...
    if "subjects_list" in group_data:  # heterogeneous (Control RS)
        ...
        subjects_with_marker = [
            s for s in group_data["subjects_list"] if marker_name in s["marker_names"]
        ]
        ...
        topos_orig_stacked = np.array(
            [s["topos_original"][marker_idx] for s in subjects_with_marker]
        )[:, np.newaxis, :]
        topos_recon_stacked = np.array(
            [s["topos_reconstructed"][marker_idx] for s in subjects_with_marker]
        )[:, np.newaxis, :]
        ...
        topos_recon_stacked = group_data["topos_recon"][
...
def perform_spearman_fdr_test(
    topos_orig_all, topos_recon_all, marker_idx, marker_name, group_name
):
    ...
    if HAS_STATSMODELS:
        ...
        print(
            f"  FDR: {marker_name} {group_name}: {significant_count}/{len(p_values_corrected)} significant channels"
        )
    else:
...
def compute_spearman_correlations(
    ...
    print(
        f"  Corr: {marker_name} {group_name}: mean |ρ| = {mean_corr:.3f}, max |ρ| = {max_corr:.3f}"
    )

    return correlation_values


def compute_spearman_correlations_with_pvalues(
    topos_orig_all, topos_recon_all, marker_idx, marker_name, group_name
):
    ...
    print(
        f"  Corr: {marker_name} {group_name}: mean |ρ| = {mean_corr:.3f}, max |ρ| = {max_corr:.3f}, sig electrodes = {n_significant}/{n_channels}"
    )

    return correlation_values, p_values_fdr


def perform_mutual_information(
    topos_orig_all, topos_recon_all, marker_idx, marker_name, group_name
):
    ...
    if n_subjects < 5:
        print(
            f"  ⚠️  {marker_name} {group_name}: insufficient subjects ({n_subjects}), returning zeros"
        )
    ...
    print(
        f"  MI: {marker_name} {group_name}: mean MI = {mean_mi:.3f}, max MI = {max_mi:.3f}"
    )
...
def create_biomarker_comparison_grid(output_dir):
    ...
    patient_labels_file = (
        "/data/project/eeg_foundation/data/metadata/patient_labels_with_controls.csv"
    )
    ...
    for group_name, data_dir in data_dirs.items():
        ...
        if allow_heterogeneous:
            ...
            if subjects_list is not None:
                ...
                print(
                    f"   ✓ {group_name}: {len(subjects_list)} subjects loaded (heterogeneous)"
                )
    ...
    for marker_name, display_name in biomarker_specs:
        ...
        available_in_any_group = any(
            marker_name in markers for markers in group_markers.values()
        )
        if available_in_any_group:
            available_biomarkers.append((marker_name, display_name))
            print(
                f"   ✓ {display_name}: available in {[g for g, markers in group_markers.items() if marker_name in markers]}"
            )
        else:
    ...
    print(
        f"📊 Creating grid plot with {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
    ...
    info, sphere, outlines = _setup_montage_and_sphere(
        topos_orig_ref.shape[2], topos_orig_ref_mean
    )
    ...
    fig, axes = plt.subplots(
        n_biomarkers, n_groups, figsize=(20, max(16, n_biomarkers * 2))
    )
    ...
    for row, (marker_name, display_name) in enumerate(available_biomarkers):
        ...
        for col, group_name in enumerate(group_names):
            ...
            marker_orig_data, marker_recon_data, n_subjects_for_marker = (
                get_marker_data_for_group(group_data, marker_name)
            )
            ...
            if marker_orig_data is not None:  # Marker exists in this group
                ...
                p_map[(p_values_corrected >= 0.01) & (p_values_corrected < 0.05)] = (
                    1  # gray: 0.01 ≤ p < 0.05
                )
                ...
                if col == n_groups - 1:
                    cbar = plt.colorbar(
                        im, ax=ax, shrink=0.6, aspect=20, ticks=[0, 1, 2]
                    )
...
def create_biomarker_comparison_grid_spearman_values_extended_with_fdr(output_dir):
    ...
    print("=" * 60)
    print(
        "Creating Extended Biomarker Group Comparison Grid (Spearman Values with FDR)"
    )
    print("=" * 60)
    ...
    print(
        f"📊 Creating extended Spearman values grid plot with FDR correction ({len(available_biomarkers)} biomarkers × {len(groups_data)} groups)"
    )
    ...
    n_groups = len(groups_data)
    fig, axes = plt.subplots(
        n_biomarkers, n_groups, figsize=(12, max(16, n_biomarkers * 1.5))
    )
    ...
    output_path = op.join(
        output_dir,
        "biomarker_group_comparison_grid_spearman_values_extended_with_fdr.png",
    )
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()

    print(
        f"✅ Extended biomarker group comparison grid (Spearman values with FDR) saved to: {output_path}"
    )
    return output_path
...
def create_biomarker_comparison_grid_spearman_values_extended(output_dir):
    ...
    print(
        f"📊 Creating extended Spearman values grid plot with {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
    ...
    os.makedirs(output_dir, exist_ok=True)
    output_path = op.join(
        output_dir, "biomarker_group_comparison_grid_spearman_values_extended.png"
    )
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()

    print(
        f"✅ Extended biomarker group comparison grid (Spearman values) saved to: {output_path}"
    )
    return output_path
...
def create_biomarker_comparison_grid_spearman_values(output_dir):
    ...
    print(
        f"📊 Creating Spearman values grid plot with {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
    ...
    # Set up montage and sphere
    info, sphere, outlines = _setup_montage_and_sphere(
        topos_orig_ref_mean.shape[1], topos_orig_ref_mean
    )
    ...
    os.makedirs(output_dir, exist_ok=True)
    output_path = op.join(
        output_dir, "biomarker_group_comparison_grid_spearman_values.png"
    )
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()

    print(
        f"✅ Biomarker group comparison grid (Spearman values) saved to: {output_path}"
    )
    return output_path
...
def create_biomarker_comparison_grid_wilcoxon(output_dir):
    ...
    print(
        f"📊 Creating Wilcoxon grid plot with {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
...
def create_biomarker_comparison_grid_mi(output_dir):
    ...
    print(
        f"📊 Creating MI grid plot with {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
    ...
    print(
        f"✅ Biomarker group comparison grid (Mutual Information) saved to: {output_path}"
    )
    return output_path
...
def main():
    ...
    # Configuration
    output_dir = (
        "/data/project/eeg_foundation/src/doc_benchmark/results/new_results/OHBM"
    )
    ...
    spearman_values_plot_path = create_biomarker_comparison_grid_spearman_values(
        output_dir
    )
    ...
    if spearman_p05_boxplot_path:
        print(
            f"✅ Spearman correlation boxplot with p < 0.05 saved: {spearman_p05_boxplot_path}"
        )
    else:
...
def create_mutual_information_boxplot(output_dir):
    ...
    print(
        f"📊 Computing subject-level MI for {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
    ...
    for marker_name, display_name in available_biomarkers:
        ...
        for group_name, group_data in groups_data.items():
            ...
            print(
                f"    {group_name}: {len(mi_values)} subjects, mean MI = {np.mean(mi_values):.3f}"
                if mi_values
                else f"    {group_name}: 0 subjects"
            )

    # Create boxplot figure
    n_biomarkers = len(available_biomarkers)
    fig, axes = plt.subplots(
        n_biomarkers, 1, figsize=(22, max(16, n_biomarkers * 2)), sharex=True
    )
    ...
    for row, (marker_name, display_name) in enumerate(available_biomarkers):
        ...
        if boxplot_data:
            ...
            for i, (group_name, values) in enumerate(
                [(lbl, data) for lbl, data in zip(boxplot_labels, boxplot_data)]
            ):
...
def create_spearman_correlation_boxplot(output_dir):
    ...
    print(
        f"📊 Computing subject-level Spearman correlation for {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
    ...
    for marker_name, display_name in available_biomarkers:
        ...
        for group_name, group_data in groups_data.items():
            ...
            print(
                f"    {group_name}: {len(spearman_values)} subjects, mean Spearman = {np.mean(spearman_values):.3f}"
                if spearman_values
                else f"    {group_name}: 0 subjects"
            )
    ...
    for row, (marker_name, display_name) in enumerate(available_biomarkers):
        ...
        if boxplot_data:
            ...
            ax.set_ylim(
                [-1.05, 1.05]
            )  # Consistent y-axis for all rows (correlation range [-1,1] with padding)
...
def create_spearman_correlation_boxplot_p05(output_dir):
    ...
    if len(all_p_values) > 0:
        ...
        print(
            f"   Significant with p < 0.05: {n_significant}/{len(all_p_values)} comparisons"
        )
    else:
    ...
    fig, axes = plt.subplots(
        n_biomarkers, 1, figsize=(18, max(18, n_biomarkers * 2.5)), sharex=True
    )
    ...
    for row, (marker_name, display_name) in enumerate(available_biomarkers):
        ...
        if boxplot_data:
            ...
            if marker_name in significant_comparisons:
                for idx, (group1, group2, p_value) in enumerate(
                    significant_comparisons[marker_name]
                ):
    ...
    fig.text(
        0.5,
        0.02,
        "* Significant with p < 0.05 (no multiple comparison correction)",
        ha="center",
        va="bottom",
        fontsize=12,
        style="italic",
...
def create_biomarker_difference_grid(output_dir):
    ...
    print(
        f"📊 Creating difference grid plot with {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
    ...
    n_groups = len(groups_data)
    fig, axes = plt.subplots(
        n_biomarkers, n_groups, figsize=(15, max(16, n_biomarkers * 1.5))
    )
    ...
    for row, (marker_name, display_name) in enumerate(available_biomarkers):
        ...
        for col, group_name in enumerate(group_names):
            ...
            if marker_orig_data is not None:  # Marker exists in this group
                ...
                mean_orig = np.mean(
                    marker_orig_data[:, 0, :], axis=0
                )  # shape: (n_channels,)
                mean_recon = np.mean(
                    marker_recon_data[:, 0, :], axis=0
                )  # shape: (n_channels,)
...
def create_biomarker_difference_grid_selected(output_dir):
    ...
    print(
        f"📊 Creating selected difference grid plot with {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
    ...
    n_groups = len(groups_data)
    fig, axes = plt.subplots(
        n_biomarkers, n_groups, figsize=(14, max(16, n_biomarkers * 2))
    )
        ...
        for col, group_name in enumerate(group_names):
            ...
            if marker_orig_data is not None:  # Marker exists in this group
                ...
                if col == n_groups - 1:
                    ...
                    cbar.ax.tick_params(labelsize=20)
                    cbar.set_label(
                        "Δ", fontsize=20, rotation=0, labelpad=15
                    )  # Increase labelpad value
    ...
    os.makedirs(output_dir, exist_ok=True)
    output_path = op.join(
        output_dir, "biomarker_group_comparison_grid_differences_selected.png"
    )
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()

    print(
        f"✅ Selected biomarker group comparison grid (differences) saved to: {output_path}"
    )
    return output_path
...
def create_biomarker_comparison_grid_selected_fdr(output_dir):
    ...
    print(
        f"📊 Creating selected FDR grid plot with {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
    ...
    os.makedirs(output_dir, exist_ok=True)
    output_path = op.join(
        output_dir, "biomarker_group_comparison_grid_selected_fdr.png"
    )
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def create_biomarker_comparison_grid_spearman_values_selected(output_dir):
    ...
    print(
        f"📊 Creating selected spearman values grid plot with {len(available_biomarkers)} biomarkers × {len(groups_data)} groups"
    )
    ...
    n_groups = len(groups_data)
    fig, axes = plt.subplots(
        n_biomarkers, n_groups, figsize=(14, max(16, n_biomarkers * 1.5))
    )
    ...
    os.makedirs(output_dir, exist_ok=True)
    output_path = op.join(
        output_dir, "biomarker_group_comparison_grid_spearman_values_selected.png"
    )
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()

    print(
        f"✅ Selected biomarker group comparison grid (Spearman values) saved to: {output_path}"
    )
    return output_path
...
def create_biomarker_comparison_grid_spearman_values_selected_with_rs(output_dir):
    ...
    n_groups = len(groups_data)
    fig, axes = plt.subplots(
        n_biomarkers, n_groups, figsize=(16, max(16, n_biomarkers * 1.5))
    )
    ...
    output_path = op.join(
        output_dir,
        "biomarker_group_comparison_grid_spearman_values_selected_with_rs.png",
    )
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()

    print(
        f"✅ Selected biomarker group comparison grid (Spearman values with Control RS) saved to: {output_path}"
    )
    return output_path
