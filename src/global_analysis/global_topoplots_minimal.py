...
class GlobalTopoAnalyzer:
    ...
    def _load_patient_labels(self):
        ...
        try:
            ...
            print(
                f"   ✓ Loaded labels for {len(self.patient_labels_original)} subject/sessions"
            )
    ...
    def collect_subject_data(self):
        ...
        subject_dirs = [
            d for d in os.listdir(computed_data_dir) if d.startswith("sub-")
        ]
        ...
        for subject_dir in sorted(subject_dirs):
            ...
            for session_dir in sorted(session_dirs):
                ...
                self._process_subject_session(
                    subject_id, session_dir, subject_session_id, session_path
                )
    ...
    def _process_subject_session(
        self, subject_id, session_id, subject_session_id, session_path
    ):
        ...
        try:
            ...
            topos_orig_file = op.join(
                orig_dir, f"topos_sub-{subject_id}_{session_id}.npz"
            )
            topos_recon_file = op.join(
                recon_dir, f"topos_sub-{subject_id}_{session_id}.npz"
            )
            ...
            if not op.exists(topos_recon_file):
                print(
                    f"     ⏭️  Skipping {subject_session_id}: missing recon topos file"
                )
                return
            ...
            if topos_orig.ndim != 2 or topos_recon.ndim != 2:
                raise ValueError(
                    f"Expected 2D topographic data, got orig: {topos_orig.shape}, recon: {topos_recon.shape}"
                )

            if topos_orig.shape != topos_recon.shape:
                raise ValueError(
                    f"Shape mismatch between orig and recon: orig {topos_orig.shape} vs recon {topos_recon.shape}"
                )
            ...
            print(
                f"     ✓ Loaded {subject_session_id}: {topos_orig.shape[0]} markers × {topos_orig.shape[1]} channels"
            )
    ...
    def prepare_global_data(self):
        ...
        for subject_id in subjects:
            ...
            if expected_shape is None:
                ...
                print(
                    f"  ⚠️  Skipping {subject_id}: shape mismatch {current_shape} vs expected {expected_shape}"
                )
                continue
        ...
        self.global_topo_data["topos_orig_all"] = np.array(
            self.global_topo_data["topos_orig_all"]
        )
        self.global_topo_data["topos_recon_all"] = np.array(
            self.global_topo_data["topos_recon_all"]
        )

        # Set dimensions from first subject
        self.global_topo_data["n_markers"] = self.global_topo_data[
            "topos_orig_all"
        ].shape[1]
        self.global_topo_data["n_channels"] = self.global_topo_data[
            "topos_orig_all"
        ].shape[2]

        # Compute mean across subjects (markers × channels)
        self.global_topo_data["topos_orig_mean"] = np.mean(
            self.global_topo_data["topos_orig_all"], axis=0
        )
        self.global_topo_data["topos_recon_mean"] = np.mean(
            self.global_topo_data["topos_recon_all"], axis=0
        )
        ...
        print(
            f"  Topo dimensions: {self.global_topo_data['n_markers']} markers × {self.global_topo_data['n_channels']} channels"
        )
        print(
            f"  Data shape: {self.global_topo_data['topos_orig_all'].shape} (subjects × markers × channels)"
        )

    def _create_custom_biomarker_topomap(
        self,
        topos_orig_mean,
        topos_recon_mean,
        info,
        ...
        fig, axes = plt.subplots(
            n_biomarkers, 3, figsize=(20, max(12, n_biomarkers * 2.5))
        )
        ...
        # Plot each biomarker
        for row, (marker_idx, label) in enumerate(
            zip(biomarker_indices, biomarker_labels)
        ):
        ...
        # Save plot
        plt.savefig(
            op.join(self.plots_dir, "custom_biomarkers_orig_recon_diff.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _create_diagnosis_filtered_biomarker_topomap(
        self,
        topos_orig_all,
        topos_recon_all,
        info,
        ...
        if diagnosis_group is None or not self.patient_labels_original:
            print(
                "  ⚠️  No diagnosis filtering available, skipping diagnosis-filtered plots"
            )
            return

        group_name = (
            "_".join(diagnosis_group).replace("+", "plus").replace("-", "minus")
        )
        print(
            f"  🎯 Creating diagnosis-filtered biomarker plots for {diagnosis_group}..."
        )
        ...
        print(
            f"     ✓ Found {len(filtered_indices)} subjects with diagnoses {diagnosis_group}"
        )
    ...
    def _create_custom_full_biomarker_topomap_wilcoxon(
        self,
        topos_orig_mean,
        topos_recon_mean,
        info,
        ...
        fig, axes = plt.subplots(
            n_biomarkers, 4, figsize=(25, max(12, n_biomarkers * 2.5))
        )
        ...
        for row, (marker_idx, label) in enumerate(
            ...
            if topos_orig_all is not None and topos_recon_all is not None:
                ...
                cbar4 = plt.colorbar(
                    im4, ax=axes[row, 3], shrink=0.6, aspect=20, ticks=[0, 1, 2]
                )
        ...
        # Save plot
        plt.savefig(
            op.join(self.plots_dir, "custom_full_biomarkers_orig_recon_wilcoxon.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _create_custom_full_biomarker_topomap_spearman(
        self,
        topos_orig_mean,
        topos_recon_mean,
        info,
        ...
        # Save plot
        plt.savefig(
            op.join(self.plots_dir, "custom_full_biomarkers_orig_recon_spearman.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        ...
        print(
            f"  ✅ Custom FULL biomarker plot (Spearman) saved with {n_biomarkers} markers"
        )

    def _create_custom_full_biomarker_topomap_wilcoxon_corrected(
        self,
        topos_orig_mean,
        topos_recon_mean,
        info,
        ...
        # Plot each biomarker
        for row, (marker_idx, label, p_val_corr) in enumerate(
            zip(biomarker_indices, biomarker_labels, p_values_corrected)
        ):
            orig_data = topos_orig_mean[marker_idx, :]
            recon_data = topos_recon_mean[marker_idx, :]
            diff_data = orig_data - recon_data
        ...
        plt.savefig(
            op.join(
                self.plots_dir,
                "custom_full_biomarkers_orig_recon_wilcoxon_corrected.png",
            ),
            dpi=300,
            bbox_inches="tight",
        )
        ...
        print(
            f"  ✅ Custom FULL biomarker plot (FDR corrected) saved with {n_biomarkers} markers"
        )
    ...
    def create_mne_topomap_plots(self):
        ...
        print(
            f"  📊 Data summary: {n_subjects} subjects, {n_markers} markers, {n_channels} channels"
        )
        ...
        # 1. All subjects plot
        print("  🎯 Creating plot for all subjects...")
        self._create_custom_biomarker_topomap(
            topos_orig_mean, topos_recon_mean, info, marker_names, sphere, outlines
        )

        # 2. Diagnosis-specific plots
        print("  🎯 Creating diagnosis-filtered plots...")
        self._create_diagnosis_filtered_biomarker_topomap(
            topos_orig_all,
            topos_recon_all,
            info,
            marker_names,
            diagnosis_group=["UWS", "VS"],
            sphere=sphere,
            outlines=outlines,
        )
        self._create_diagnosis_filtered_biomarker_topomap(
        ...
        self._create_custom_full_biomarker_topomap_wilcoxon(
            topos_orig_mean,
            topos_recon_mean,
            info,
            marker_names,
        ...
        self._create_custom_full_biomarker_topomap_spearman(
            topos_orig_mean,
            topos_recon_mean,
            info,
            marker_names,
        ...
        self._create_custom_full_biomarker_topomap_wilcoxon_corrected(
            topos_orig_mean,
            topos_recon_mean,
            info,
            marker_names,
        ...
        print("  ✅ Created 6 custom biomarker topographic plots")
    ...
    def run_analysis(self):
        ...
        print("Created 6 plots:")
        print("  1. custom_biomarkers_orig_recon_diff.png")
        print("  2. custom_biomarkers_orig_recon_diff_MCSplus_MCSminus.png")
        print("  3. custom_biomarkers_orig_recon_diff_UWS_VS.png")
        print("  4. custom_full_biomarkers_orig_recon_wilcoxon.png")
        print("  5. custom_full_biomarkers_orig_recon_spearman.png")
        print("  6. custom_full_biomarkers_orig_recon_wilcoxon_corrected.png")
        print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Global topographic analysis - Minimal version for 6 plots"
    )
    parser.add_argument(
        "--results-dir",
    ...
    patient_labels_file = (
        args.patient_labels if op.exists(args.patient_labels) else None
    )

    print(
        f"Starting minimal global topographic analysis at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )

    analyzer = GlobalTopoAnalyzer(
        args.results_dir, args.output_dir, patient_labels_file
    )
