...
class CrossSubjectClassifier:
    ...
    def load_patient_labels(self):
        ...
        try:
            ...
            print(
                "    Binary classification: VS (Vegetative State) vs MCS (Minimally Conscious State)"
            )
    ...
    def get_marker_names(self):
        ...
        for name in marker_names:
            ...
            if name.startswith("delta_power_spectralpower_"):
                ...
                abbrev = name.replace(
                    "kolmogorov_complexity_kolmogorovcomplexity_", "KC_"
                )
    ...
    def load_feature_names(self):
        ...
        if op.exists(self.data_dir):
            subject_dirs = [
                d for d in os.listdir(self.data_dir) if d.startswith("sub-")
            ]
            ...
            for subject_dir in sorted(subject_dirs):
                ...
                if "_" in subject_dir and (
                    subject_dir.endswith("_original") or subject_dir.endswith("_recon")
                ):
                    ...
                    # Look for .npz file
                    npz_filename = (
                        f"scalars_{subject_id_raw}_{subject_dir.split('_')[-1]}.npz"
                    )
                        ...
                        try:
                            ...
                            for name in feature_names:
                                ...
                                if name.startswith("PowerSpectralDensity_"):
                                    abbrev = name.replace(
                                        "PowerSpectralDensity_", "PSD_"
                                    )
                                elif name.startswith("PowerSpectralDensitySummary_"):
                                    abbrev = name.replace(
                                        "PowerSpectralDensitySummary_", "PSD_Sum_"
                                    )
                            ...
                            print(
                                f"   ✓ Loaded {len(feature_names)} feature names from {npz_filename}"
                            )
                            print(
                                f"   Features: {', '.join(abbreviated_names[:5])}{'...' if len(abbreviated_names) > 5 else ''}"
                            )
        ...
        for subject_dir in sorted(subject_dirs):
            ...
            try:
                session_dirs = [
                    d for d in os.listdir(subject_path) if d.startswith("ses-")
                ]
            ...
            for session_dir in sorted(session_dirs):
                ...
                if op.exists(metrics_file):
                    try:
                        ...
                        print(
                            f"   ✓ Loaded {len(feature_names)} feature names from {subject_dir}/{session_dir}"
                        )
                        print(
                            f"   Features: {', '.join(abbreviated_names[:5])}{'...' if len(abbreviated_names) > 5 else ''}"
                        )
        ...
        if self.marker_type == "scalar":
            ...
            print(
                f"   ✓ Using {len(marker_names)} standard topo marker names (will be expanded per channel)"
            )
            return marker_names, abbreviated_names
                ...
                if "scalars" in data_dict:
                    ...
                    if len(keys) > 0:
                        ...
                        if all_scalars:
                            ...
                            print(
                                f"      Collected {len(keys)} scalar markers from .npz file"
                            )
                        else:
    ...
    def collect_data(self):
        ...
        print(
            f"🔍 Collecting {self.marker_type} {self.data_origin} data from subjects..."
        )
        ...
        try:
            ...
            raise ValueError(
                f"Permission denied accessing data directory {self.data_dir}: {e}"
            )
            ...
            if "_" in subject_dir and (
                ...
                if len(subject_data) > 0:
                    ...
                    if marker_data.shape != expected_shape:
                        ...
                        print(
                            f"       Expected: {expected_shape}, got: {marker_data.shape}"
                        )
                        subjects_skipped += 1
                        continue
                ...
                print(
                    f"   ✓ Loaded {subject_session_key}: {marker_data.shape} features, state={labels_dict[subject_session_key]}"
                )
                ...
                for session_dir in sorted(session_dirs):
                    ...
                    if marker_data is None:
                        print(
                            f"   ⏭️  Skipping {subject_session_key}: failed to load data"
                        )
                        subjects_skipped += 1
                        continue
                    ...
                    if len(subject_data) > 0:
                        ...
                        if marker_data.shape != expected_shape:
                            print(
                                f"   ⚠️  Skipping {subject_session_key}: shape mismatch"
                            )
                            print(
                                f"       Expected: {expected_shape}, got: {marker_data.shape}"
                            )
        ...
        print(
            f"   Final dataset: {self.X.shape[0]} subjects × {self.X.shape[1]} features"
        )
        ...
        if min_class_size == 0:
            ...
            print(
                "    Warning: Some classes have only 1 sample. Consider using Leave-One-Out CV."
            )
    ...
    def create_pipeline(self, n_estimators=500, max_depth=4):
        ...
        class_weights = compute_class_weight(
            "balanced", classes=np.unique(self.y_encoded), y=self.y_encoded
        )
    ...
    def evaluate_model(
        self, pipeline, cv_strategy="stratified", n_splits=4, test_size=0.2
    ):
        ...
        print(
            f" Evaluating model with {cv_strategy} cross-validation and {test_size:.0%} test set..."
        )
        ...
        try:
            ...
            print(
                f"   ✓ Using StratifiedGroupKFold with {n_splits} splits (maintains class balance)"
            )
            ...
            train_idx, test_idx = next(
                gss.split(self.X, self.y_encoded, groups=subject_groups)
            )
        ...
        if overlap:
            raise ValueError(
                f"❌ CRITICAL: Subject leakage detected! Subjects in both train and test: {overlap}"
            )

        print(
            f"    ✓ No subject leakage verified: {len(unique_train_subjects)} unique subjects in train, {len(unique_test_subjects)} in test"
        )

        print(
            f"   ✓ Train set: {X_train.shape[0]} sessions from {len(unique_train_subjects)} unique subjects"
        )
        print(
            f"   ✓ Test set: {X_test.shape[0]} sessions from {len(unique_test_subjects)} unique subjects"
        )
        ...
        if cv_strategy == "loo":
            ...
            if max_possible_splits < 2:
                print(
                    f"   WARNING: Only {n_unique_train_subjects} unique subjects in training, cannot perform CV"
                )
                cv = LeaveOneOut()
                print(
                    f"   Falling back to Leave-One-Out CV on training set ({n_train_samples} folds)"
                )
            else:
                ...
                try:
                    ...
                    print(
                        f"   🔒 Using GroupKFold with {effective_n_splits} splits (respects subject groups only)"
                    )
        else:
        ...
        if len(test_classes_present) == 2:
            ...
            print(
                f"   Only one class present in test set: {test_class_names_present[0]}. Cannot compute AUC."
            )
            test_auc_score = None
        else:
            print(
                f"   More than 2 classes found: {test_class_names_present}. This should not happen for binary classification."
            )
            test_auc_score = None
    ...
    def plot_results(self, results):
        ...
        fig.suptitle(
            f"Cross-Subject Classification Results\n"
        ...
        ax.set_title(
            f"CV Scores on Training Set ({results['cv_strategy']})\n"
        ...
        test_class_names_present = results.get(
            "test_class_names_present", self.class_names
        )
        ...
        thresh = conf_matrix.max() / 2.0 if conf_matrix.size > 0 else 0
        ...
        if self.feature_names_abbreviated is not None and len(
    ...
    def _plot_multiclass_roc_curves(self, results):
        ...
        if len(test_classes_present) < 2:
            ...
            plt.savefig(plot_file, dpi=150, bbox_inches="tight")
            print(
                f"   ROC curves saved to: {plot_file} (insufficient classes for proper ROC)"
            )
            plt.close()
            return
        ...
        for i, (class_idx, class_name, color) in enumerate(
            zip(test_classes_present, test_class_names_present, colors)
        ):
            # For multiclass, we need to get the probability for this specific class
            class_proba = (
                y_test_proba[:, i]
                if i < y_test_proba.shape[1]
                else y_test_proba[:, class_idx]
            )
            ...
            if len(test_classes_present) == 2:
                ...
                y_true_binary = (
                    ...
                    else (y_test_true == class_idx).astype(int)
                )
        ...
        if auc_scores:
            print(
                f"   AUC scores: {', '.join([f'{k}: {v:.3f}' for k, v in auc_scores.items()])}"
            )
        plt.close()
    ...
    def save_results(self, results, pipeline):
        ...
        json_results = {
            ...
            "performance_metrics": {
                ...
                "auc_score": float(results["test_auc_score"])
                if results["test_auc_score"] is not None
                else None,
            },
            "class_metrics": {
            ...
            "feature_names_abbreviated": self.feature_names_abbreviated
            if self.feature_names_abbreviated is not None
            else [],
            "subjects_info": {
                "train_subjects": results["subjects_train"],
                "test_subjects": results["subjects_test"],
                "test_true_labels": [
                    self.class_names[i] for i in results["y_test_true"]
                ],
                "test_predicted_labels": [
                    self.class_names[i] for i in results["y_test_pred"]
                ],
        ...
        subject_results_df = pd.DataFrame(
            {
                ...
                "correct_prediction": [
                    true == pred
                    for true, pred in zip(test_true_labels, test_pred_labels)
                ],
                "dataset_split": "test",
            }
        )
        ...
        if self.feature_names is not None and len(self.feature_names) == len(
            ...
            feature_importance_file = op.join(
                self.output_dir, "feature_importances.csv"
            )
            ...
            feature_importance_df.to_csv(feature_importance_file, index=False)
            print(
                f"   ✓ Feature importances saved to: {feature_importance_file} (with indices)"
            )
    ...
    def run_classification(
        ...
        try:
            ...
            print("=" * 80)
            print(
                f"Dataset: {results['n_train_subjects']} train + {results['n_test_subjects']} test subjects, {results['n_features']} features"
            )
            print(f"Classes: {', '.join(results['class_names'])}")
            print(
                f"CV Balanced Accuracy (train): {results['cv_mean']:.3f} ± {results['cv_std']:.3f}"
            )
...
class CrossDataClassifier:
            ...
            try:
                ...
                def _load_array_filtered(path, marker_names):
                    if path.endswith(".npz"):
                        ...
                        if len(keys) == 1:
                            print(
                                f"    Warning: Single array found in {path}, cannot filter individual markers"
                            )
                            return (
                                data_dict[keys[0]],
                                None,
                        ...
                        if missing_markers:
                            print(
                                f"    Skipping {path}: missing {len(missing_markers)} markers: {missing_markers[:5]}{'...' if len(missing_markers) > 5 else ''}"
                            )
                            return (
                                None,
                                None,
                                missing_markers,
                            )  # Return missing markers for logging

                        return (
                            np.array(filtered_values),
                            None,
                            [],
                        )  # Return data, None for missing, empty missing list
                    else:
                ...
                if data_orig_result[0] is None or data_recon_result[0] is None:
                    missing_orig = (
                        data_orig_result[2] if len(data_orig_result) > 2 else []
                    )
                    missing_recon = (
                        data_recon_result[2] if len(data_recon_result) > 2 else []
                    )
                ...
                return (
                    ...
                    data_recon,
                    [],
                )  # Return 3-tuple for consistency (no missing markers)
    ...
    def collect_data_split_dirs(self):
        ...
        if common_keys:
            ...
            try:
                ...
                def _has_nan_in_npz_key(filepath, key):
                    ...
                    major = raw[6]
                    hlen = (
                        _struct.unpack("<H", raw[8:10])[0]
                        if major == 1
                        else _struct.unpack("<I", raw[8:12])[0]
                    )
                    ...
                    for i in range(0, len(arr_bytes) - itemsize + 1, itemsize):
                        v = _struct.unpack(
                            "f" if itemsize == 4 else "d", arr_bytes[i : i + itemsize]
                        )[0]
                ...
                for k in candidate_keys:
                    if _has_nan_in_npz_key(first_recon_fp, k) or _has_nan_in_npz_key(
                        first_orig_fp, k
                    ):
                        nan_keys.append(k)
                    else:
                ...
                if nan_keys:
                    print(
                        f"   ⚠️  Dropped {len(nan_keys)} keys with NaN/Inf in first sample "
                        f"(std across epochs undefined — likely single-epoch subjects):"
                    )
                    print(
                        f"      e.g. {nan_keys[:4]}{'...' if len(nan_keys) > 4 else ''}"
                    )

                effective_marker_names = finite_keys
                print(
                    f"   Effective marker keys   : {len(effective_marker_names)} "
                    f"(intersection of both file key sets, NaN keys excluded)"
                )
                print(
                    f"   Keys: {effective_marker_names[:5]}{'...' if len(effective_marker_names) > 5 else ''}"
                )
                ...
                print(
                    f"   Warning: could not determine key intersection ({e}); "
        ...
        if len(subject_data_orig) > 1:
            ...
            if not all(
                d.shape == first_shape_orig for d in subject_data_orig
            ) or not all(d.shape == first_shape_recon for d in subject_data_recon):
                raise ValueError(
            ...
            print(
                f"    ✓ All data validated: {first_shape_orig[0]} features per subject"
            )
        ...
        print(
            f"   Final dataset: {self.X_original.shape[0]} subjects × {self.X_original.shape[1]} features"
        )
        print(f"   Classes: {list(self.class_names)}")
    ...
    def load_original_from_baseline_csv(self, effective_csv_cols=None):
        ...
        print(
            f"   Filtered to Reduction='{self.BASELINE_CSV_REDUCTION_A}': {len(df)} rows"
        )
        ...
        if missing_cols:
            raise ValueError(f"Baseline CSV missing expected columns: {missing_cols}")
    ...
    def collect_data_baseline_csv(self):
        ...
        print(
            f"   Effective markers: {len(effective_internal)} / "
        ...
        for subject_session_key in common_keys:
            ...
            try:
                ...
                if missing:
                    print(
                        f"    Skipping {subject_session_key}: recon missing "
            ...
            if orig_values.shape != recon_values.shape:
                print(
                    f"    Skipping {subject_session_key}: shape mismatch "
            ...
            collected_subjects.append(subject_session_key)
            print(
                f"   ✓ Loaded {subject_session_key}: {orig_values.shape[0]} features, "
                f"state={labels_dict[subject_session_key]}"
            )
        ...
        print(
            f"   Final dataset: {self.X_original.shape[0]} subjects × "
    ...
    def collect_data(self):
        ...
        print(
            f"🔍 Collecting {self.marker_type} data (both original and reconstructed) from subjects..."
        )
        print(f"   Data directory: {self.data_dir}")
        ...
        for subject_dir in sorted(subject_dirs):
            ...
            if subject_dir.startswith("sub-") and "_" not in subject_dir:
                ...
                for session_dir in sorted(session_dirs):
                    ...
                    marker_data_orig, marker_data_recon, missing_markers = (
                        self.load_subject_data_both(
                            session_path, orig_filepath, recon_filepath
                        )
                    )

                    if marker_data_orig is None or marker_data_recon is None:
                        print(
                            f"   ⏭️  Skipping {subject_session_key}: failed to load both data types"
                        )
                        discarded_subjects.append(subject_session_key)
                        subjects_skipped += 1
                        continue
                    ...
                    if len(subject_data_orig) > 0:
                        ...
                        if marker_data_orig.shape != expected_shape_orig:
                            ...
                            print(
                                f"       Expected orig: {expected_shape_orig}, got: {marker_data_orig.shape}"
                            )
                            subjects_skipped += 1
                            continue
                        ...
                        if marker_data_recon.shape != expected_shape_recon:
                            ...
                            print(
                                f"       Expected recon: {expected_shape_recon}, got: {marker_data_recon.shape}"
                            )
                            subjects_skipped += 1
                            continue
                    ...
                    print(
                        f"   ✓ Loaded {subject_session_key}: orig={marker_data_orig.shape}, recon={marker_data_recon.shape}, state={labels_dict[subject_session_key]}"
                    )
                ...
                for session_dir in sorted(session_dirs):
                    ...
                    # Load both original and reconstructed marker data
                    marker_data_orig, marker_data_recon, missing_markers = (
                        self.load_subject_data_both(session_path)
                    )
        ...
        if hasattr(self, "_temp_data_storage"):
            ...
            for subject_key, data_info in self._temp_data_storage.items():
                ...
                for (
                    other_subject_key,
                    other_data_info,
                ) in self._temp_data_storage.items():
                    if other_data_info[
                        "data_type"
                    ] == other_type and subject_key.replace(
                        "_ses-01", ""
                    ) == other_subject_key.replace("_ses-01", ""):
                ...
                if other_key is not None:
                    ...
                    if len(subject_data_orig) > 0:
                        ...
                        if orig_data.shape != expected_shape_orig:
                            print(
                                f"   ⚠️  Skipping {subject_key}: original shape mismatch"
                            )
                            continue

                        if recon_data.shape != expected_shape_recon:
                            print(
                                f"   ⚠️  Skipping {subject_key}: reconstructed shape mismatch"
                            )
                            continue
                    ...
                    print(
                        f"   ✓ Paired {subject_key}: orig={orig_data.shape}, recon={recon_data.shape}"
                    )
        ...
        if len(subject_data_orig) > 1:
            ...
            all_same_recon = all(
                d.shape == first_shape_recon for d in subject_data_recon
            )
    ...
    def _grid_search_hyperparameters(
        self, X_train, y_train, groups_train, cv_strategy, n_splits
    ):
    ...
    def run_cross_data_classification(
        self, cv_strategy="stratified", n_splits=4, test_size=0.2
    ):
        """
        ...
        print(
            "   ✓ No subject bias in cross-testing (same test subjects for all scenarios)"
        )
        ...
        try:
            ...
            print(
                "   IMPORTANT: Using identical train/test subject splits for BOTH original and reconstructed data"
            )
            print("   This ensures:")
            print(
                "     - No subject bias when comparing model performance on different data types"
            )
            ...
            print(
                "     - All sessions from same subject stay together (no session leakage)"
            )
            ...
            try:
                ...
                train_idx, test_idx = next(
                    gss.split(X_orig, y_encoded, groups=subject_groups)
                )
            ...
            print(
                f"   ✓ Test set: {X_orig_test.shape[0]} sessions from {len(unique_test_subjects)} unique subjects"
            )
            ...
            for class_idx, count in zip(unique_train, counts_train):
                print(
                    f"      {self.class_names[class_idx]}: {count} subjects ({count / len(y_train) * 100:.1f}%)"
                )
            ...
            for class_idx, count in zip(unique_test, counts_test):
                print(
                    f"      {self.class_names[class_idx]}: {count} subjects ({count / len(y_test) * 100:.1f}%)"
                )

            # CRITICAL VERIFICATION: Ensure data shapes match for same subjects
            print(f"\n   ✓ Verification: Original train shape: {X_orig_train.shape}")
            print(
                f"   ✓ Verification: Reconstructed train shape: {X_recon_train.shape}"
            )
            ...
            assert X_orig_train.shape == X_recon_train.shape, (
                "Train set shape mismatch between original and reconstructed!"
            )
            assert X_orig_test.shape == X_recon_test.shape, (
                "Test set shape mismatch between original and reconstructed!"
            )
            print(
                "   ✓ All shape verifications passed - same subjects in both data types!"
            )
            ...
            pipeline_A = self.create_pipeline(
                C=best_params_A["C"],
                kernel=best_params_A["kernel"],
                gamma=best_params_A["gamma"],
            )
            pipeline_A.fit(X_orig_train, y_train)
            print(
                f"      Model A: kernel={best_params_A['kernel']}, C={best_params_A['C']}, gamma={best_params_A['gamma']}, CV Score={best_params_A['score']:.3f}"
            )

            pipeline_B = self.create_pipeline(
                C=best_params_B["C"],
                kernel=best_params_B["kernel"],
                gamma=best_params_B["gamma"],
            )
            pipeline_B.fit(X_recon_train, y_train)
            print(
                f"      Model B: kernel={best_params_B['kernel']}, C={best_params_B['C']}, gamma={best_params_B['gamma']}, CV Score={best_params_B['score']:.3f}"
            )
            ...
            print(
                f"      SVC Model B.classes_: {pipeline_B.named_steps['svc'].classes_}"
            )
            ...
            print(
                "     1. Model A (trained on ORIGINAL) → tested on ORIGINAL test subjects"
            )
            print(
                "     2. Model A (trained on ORIGINAL) → tested on RECONSTRUCTED test subjects"
            )
            print(
                "     3. Model B (trained on RECONSTRUCTED) → tested on ORIGINAL test subjects"
            )
            print(
                "     4. Model B (trained on RECONSTRUCTED) → tested on RECONSTRUCTED test subjects"
            )
            print(
            ...
            y_pred_A_on_orig = np.argmax(
                y_proba_A_on_orig, axis=1
            )  # Use argmax for consistency
            y_pred_A_on_recon = np.argmax(y_proba_A_on_recon, axis=1)
            print(
                f"      ✓ Tested on original test set: accuracy = {accuracy_score(y_test, y_pred_A_on_orig):.3f}, balanced_accuracy = {balanced_accuracy_score(y_test, y_pred_A_on_orig):.3f}"
            )
            print(
                f"      ✓ Tested on reconstructed test set: accuracy = {accuracy_score(y_test, y_pred_A_on_recon):.3f}, balanced_accuracy = {balanced_accuracy_score(y_test, y_pred_A_on_recon):.3f}"
            )
            ...
            for i in range(min(3, len(y_test))):
                ...
                proba_vs = y_proba_A_on_orig[
                    i, np.where(self.class_names == "VS")[0][0]
                ]
                print(
                    f"      Subject {subjects_test[i]}: True={true_label}, Pred={pred_label} (encoded={y_pred_A_on_orig[i]}), P(MCS)={proba_mcs:.3f}, P(VS)={proba_vs:.3f}"
                )
            ...
            print(
                f"      ✓ Tested on original test set: accuracy = {accuracy_score(y_test, y_pred_B_on_orig):.3f}, balanced_accuracy = {balanced_accuracy_score(y_test, y_pred_B_on_orig):.3f}"
            )
            print(
                f"      ✓ Tested on reconstructed test set: accuracy = {accuracy_score(y_test, y_pred_B_on_recon):.3f}, balanced_accuracy = {balanced_accuracy_score(y_test, y_pred_B_on_recon):.3f}"
            )
            ...
            results = {
                "model_A_orig_test": self._compute_metrics(
                    y_test,
                    y_pred_A_on_orig,
                    y_proba_A_on_orig,
                    "Model A on Original Test",
                ),
                "model_A_recon_test": self._compute_metrics(
                    y_test,
                    y_pred_A_on_recon,
                    y_proba_A_on_recon,
                    "Model A on Reconstructed Test",
                ),
                "model_B_orig_test": self._compute_metrics(
                    y_test,
                    y_pred_B_on_orig,
                    y_proba_B_on_orig,
                    "Model B on Original Test",
                ),
                "model_B_recon_test": self._compute_metrics(
                    y_test,
                    y_pred_B_on_recon,
                    y_proba_B_on_recon,
                    "Model B on Reconstructed Test",
                ),
            ...
            print(
                f"   Model A (original) → Reconstructed test set:   accuracy={results['model_A_recon_test']['accuracy']:.3f}, balanced_accuracy={results['model_A_recon_test']['balanced_accuracy']:.3f}"
            )
            print(
                f"   Model B (reconstructed) → Original test set:   accuracy={results['model_B_orig_test']['accuracy']:.3f}, balanced_accuracy={results['model_B_orig_test']['balanced_accuracy']:.3f}"
            )
            print(
    ...
    def run_cross_data_full_cv(
        ...
        try:
            ...
            accum = {
                k: {"y_true": [], "y_pred": [], "y_proba": [], "subjects": []}
            ...
            for fold_idx, (train_idx, test_idx) in enumerate(
                ...
                print(
                    f"   Train: {len(train_subj)} subjects ({X_orig_train.shape[0]} sessions)"
                )
                print(
                    f"   Test : {len(test_subj)} subjects ({X_orig_test.shape[0]} sessions)"
                )
                ...
                ba_A_orig = balanced_accuracy_score(
                    y_test, np.argmax(pipeline_A.predict_proba(X_orig_test), axis=1)
                )
                ba_B_recon = balanced_accuracy_score(
                    y_test, np.argmax(pipeline_B.predict_proba(X_recon_test), axis=1)
                )
                print(
                    f"   Fold {fold_idx + 1} balanced accuracy: "
            ...
            for sk in scenario_keys:
                ...
                macro = self._compute_macro_average(
                    accum[sk]["y_true"], accum[sk]["y_pred"], accum[sk]["y_proba"], sk
                )
                results[sk]["macro_average"] = macro

            # Average feature importances
            results["feature_importances_A"] = (
                np.mean(fold_importances_A, axis=0) if fold_importances_A else None
            )
            results["feature_importances_B"] = (
                np.mean(fold_importances_B, axis=0) if fold_importances_B else None
            )
            ...
            results["y_test"] = np.concatenate(accum["model_A_orig_test"]["y_true"])
            results["test_subjects"] = accum["model_A_orig_test"]["subjects"]
            results["train_subjects"] = list(
                subjects
            )  # all subjects train at some point
            ...
            self._plot_macro_vs_micro(results, scenario_keys)
            ...
            for sk in scenario_keys:
                r = results[sk]
                micro_auc_str = f"{r['auc_score']:.3f}" if r["auc_score"] else "N/A"
                macro = r.get("macro_average", {})
                macro_ba = macro.get("balanced_accuracy", {})
                macro_auc = macro.get("auc_score", {})
                macro_ba_str = (
                    f"{macro_ba['mean']:.3f}\u00b1{macro_ba['std']:.3f}"
                    if macro_ba.get("mean") is not None
                    else "N/A"
                )
                macro_auc_str = (
                    f"{macro_auc['mean']:.3f}\u00b1{macro_auc['std']:.3f}"
                    if macro_auc.get("mean") is not None
                    else "N/A"
                )
                print(f"   {sk}:")
                print(
                    f"      Micro:  Acc={r['accuracy']:.3f}, "
                    f"Bal Acc={r['balanced_accuracy']:.3f}, "
                    f"AUC={micro_auc_str}"
                )
                print(f"      Macro:  Bal Acc={macro_ba_str}, AUC={macro_auc_str}")
            print(f"\n   Results saved to: {self.output_dir}")
            print("=" * 80)
    ...
    def _save_full_cv_results(self, results):
        ...
        for sk in scenario_keys:
            folder = (
                sk.replace("model_A_", "Model_A_")
                .replace("model_B_", "Model_B_")
                .replace("_test", "_Test")
                .replace("orig", "Original")
                .replace("recon", "Reconstructed")
            )
            ...
            json_results = {
                ...
                "performance_metrics": {
                    ...
                    "macro_average": r.get("macro_average"),
                },
                "class_metrics": {
            ...
            y_true_for_csv = results.get(
                "y_test", np.zeros(len(r["y_pred"]), dtype=int)
            )
            if subj_list and len(subj_list) == len(r["y_pred"]):
                ...
                pred_df.to_csv(
                    op.join(scenario_dir, "subject_predictions.csv"), index=False
                )

            # Feature importances
            imp_key = (
                "feature_importances_A" if "model_A" in sk else "feature_importances_B"
            )
            ...
            if (
                ...
                fi_df.to_csv(
                    op.join(scenario_dir, "feature_importances.csv"), index=False
                )
        ...
        with open(
            op.join(self.output_dir, "cross_data_results_full_cv.json"), "w"
        ) as f:
            json.dump(consolidated, f, indent=2)
        print(
            f"   ✓ Consolidated results: {self.output_dir}/cross_data_results_full_cv.json"
        )
    ...
    def _compute_metrics(self, y_true, y_pred, y_proba, description):
        ...
        if len(y_true) != len(y_pred):
            print(
                f"   ⚠️  WARNING: Length mismatch! y_true={len(y_true)}, y_pred={len(y_pred)}"
            )
        ...
        print(
            f"      Correct predictions: {np.trace(conf_matrix)}/{np.sum(conf_matrix)}"
        )
    ...
    def _compute_macro_average(
        self, accum_y_true, accum_y_pred, accum_y_proba, scenario_key
    ):
        ...
        accum_y_proba : list of np.ndarray
            One array per fold.
        scenario_key : str
            Scenario description for logging.
        ...
        for i in range(n_folds):
            fold_metrics = self._compute_metrics(
                ...
                accum_y_pred[i],
                accum_y_proba[i],
                f"{scenario_key}_fold{i}",
            )

            accuracies.append(float(fold_metrics["accuracy"]))
            balanced_accuracies.append(float(fold_metrics["balanced_accuracy"]))
            auc_val = fold_metrics["auc_score"]
            auc_scores.append(float(auc_val) if auc_val is not None else None)
            precisions[i] = fold_metrics["precision"]
            recalls[i] = fold_metrics["recall"]
            f1_scores[i] = fold_metrics["f1_score"]
            ...
            per_fold.append(
                {
                    ...
                    "auc_score": float(auc_val) if auc_val is not None else None,
                    "precision": fold_metrics["precision"].tolist(),
                    "recall": fold_metrics["recall"].tolist(),
                    "f1_score": fold_metrics["f1_score"].tolist(),
                }
            )
    ...
    def _plot_macro_vs_micro(self, results, scenario_keys):
        """Generate a separate PNG comparing micro vs macro for all scenarios."""
        # Check that at least one scenario has macro data
        has_macro = any(
            results.get(sk, {}).get("macro_average") is not None for sk in scenario_keys
        )
        if not has_macro:
            return

        fig, axes = plt.subplots(2, 2, figsize=(18, 14))
        n_folds = results.get("metadata", {}).get("n_folds", "?")
        fig.suptitle(
            f"Cross-Data SVM — Micro vs Macro Evaluation ({n_folds}-fold CV)",
            fontsize=14,
        )

        for idx, sk in enumerate(scenario_keys):
            ax = axes[idx // 2, idx % 2]
            r = results.get(sk)
            if r is None or r.get("macro_average") is None:
                ax.text(
                    0.5,
                    0.5,
                    f"{sk}\nNo macro data",
            ...
            macro = r["macro_average"]

            metric_names = ["Accuracy", "Bal. Acc", "AUC"]
            micro_vals = [
                float(r["accuracy"]),
                float(r["balanced_accuracy"]),
                float(r["auc_score"]) if r["auc_score"] is not None else 0,
            ]
            macro_means = [
                macro["accuracy"]["mean"],
                macro["balanced_accuracy"]["mean"],
                macro["auc_score"]["mean"]
                if macro["auc_score"]["mean"] is not None
                else 0,
            ]
            macro_stds = [
                macro["accuracy"]["std"],
                ...
                macro["auc_score"]["std"]
                if macro["auc_score"]["std"] is not None
                else 0,
            ]
            ...
            ax.set_ylim(0, 1.05)
            ax.set_ylabel("Score")
            ax.set_title(sk.replace("_", " ").title())
        ...
        plt.tight_layout()
        plot_file = op.join(self.output_dir, "macro_vs_micro_comparison.png")
        plt.savefig(plot_file, dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def _save_cross_data_results(self, results, pipeline_A, pipeline_B):
        ...
        for scenario_key, test_set_name, pipeline, folder_name in scenarios:
            ...
            json_results = {
                "experiment_info": {
                    ...
                    "model_trained_on": "original"
                    if "model_A" in scenario_key
                    else "reconstructed",
                    "model_tested_on": "original"
                    if "orig_test" in scenario_key
                    else "reconstructed",
                ...
                "performance_metrics": {
                    ...
                    "auc_score": float(scenario_results["auc_score"])
            ...
            print(f"      self.class_names: {self.class_names}")
            print(
                f"      First 3 encoded predictions: {scenario_results['y_pred'][:3]}"
            )
            ...
            for idx in range(min(3, len(test_pred_labels))):
                print(
                    f"      Row {idx}: y_pred={scenario_results['y_pred'][idx]}, label={test_pred_labels[idx]}, P(MCS)={scenario_results['y_proba'][idx, mcs_idx]:.3f}, P(VS)={scenario_results['y_proba'][idx, vs_idx]:.3f}"
                )
            ...
            for i, class_name in enumerate(self.class_names):
                subject_results_df[f"prob_{class_name}"] = scenario_results["y_proba"][
                    :, i
                ]
            ...
            self._plot_scenario_results(
                scenario_results, scenario_dir, scenario_key, results
            )
        ...
        consolidated_results = {
            "model_A_orig_test": {
                ...
                "auc_score": float(results["model_A_orig_test"]["auc_score"])
                if results["model_A_orig_test"]["auc_score"] is not None
                else None,
            },
            "model_A_recon_test": {
                "accuracy": float(results["model_A_recon_test"]["accuracy"]),
                "balanced_accuracy": float(
                    results["model_A_recon_test"]["balanced_accuracy"]
                ),
                "auc_score": float(results["model_A_recon_test"]["auc_score"])
                if results["model_A_recon_test"]["auc_score"] is not None
                else None,
            },
            "model_B_orig_test": {
                "accuracy": float(results["model_B_orig_test"]["accuracy"]),
                "balanced_accuracy": float(
                    results["model_B_orig_test"]["balanced_accuracy"]
                ),
                "auc_score": float(results["model_B_orig_test"]["auc_score"])
                if results["model_B_orig_test"]["auc_score"] is not None
                else None,
            },
            "model_B_recon_test": {
                ...
                "auc_score": float(results["model_B_recon_test"]["auc_score"])
                if results["model_B_recon_test"]["auc_score"] is not None
                else None,
            },
    ...
    def _plot_subject_probabilities(self, results):
        ...
        debug_df = pd.DataFrame(
            {
                ...
                "True_Label": [
                    "MCS" if results["y_test"][i] == mcs_idx else "VS"
                ...
                "Pred_OO": [
                    "MCS"
                    if results["model_A_orig_test"]["y_pred"][i] == mcs_idx
                    else "VS"
                    for i in range(n_subjects)
                ],
                "P(MCS)_RR": proba_RR,
                ...
                "Pred_RR": [
                    "MCS"
                    if results["model_B_recon_test"]["y_pred"][i] == mcs_idx
                    else "VS"
                    for i in range(n_subjects)
                ],
            }
        ...
        ax.plot(
            ...
            linewidth=2.5,
            markersize=7,
            label="OO (Original Model → Original Test)",
            color="blue",
            alpha=0.8,
        )
        ax.plot(
            ...
            linewidth=2.5,
            markersize=7,
            label="OR (Original Model → Reconstructed Test)",
            color="orange",
            alpha=0.8,
        )
        ax.plot(
            ...
            linewidth=2.5,
            markersize=7,
            label="RO (Reconstructed Model → Original Test)",
            color="green",
            alpha=0.8,
        )
        ax.plot(
            ...
            linewidth=2.5,
            markersize=7,
            label="RR (Reconstructed Model → Reconstructed Test)",
            color="red",
            alpha=0.8,
        )
        ...
        ax.set_ylabel(
            "P(MCS) - Probability of MCS Class", fontsize=13, fontweight="bold"
        )
        ax.set_title(
            f"Subject-Level Probabilities: P(MCS) from SVC.predict_proba()\\n"
        ...
        for i, (subj_idx, true_label) in enumerate(zip(x, y_true)):
            ...
            ax.axvspan(
                subj_idx - 0.4, subj_idx + 0.4, alpha=0.15, color=color, zorder=0
            )

        ax.text(
            0.02,
            0.98,
            "Background: Green = True MCS, Red = True VS",
            transform=ax.transAxes,
            fontsize=10,
            verticalalignment="top",
        ...
        plt.tight_layout()
        prob_plot_file = op.join(
            self.output_dir, "subject_probabilities_all_scenarios.png"
        )
    ...
    def _plot_model_A_probabilities(self, results):
        ...
        ax.plot(
            ...
            linewidth=3,
            markersize=8,
            label="Original Model → Original Test",
            color="blue",
            alpha=0.9,
        )
        ax.plot(
            ...
            linewidth=3,
            markersize=8,
            label="Original Model → Reconstructed Test",
            color="orange",
            alpha=0.9,
        )
        ...
        ax.set_title(
            f"Model A (Original Data): Subject-Level Probabilities\\n"
    ...
    def _plot_model_B_probabilities(self, results):
        ...
        ax.plot(
            ...
            linewidth=3,
            markersize=8,
            label="Reconstructed Model → Original Test",
            color="green",
            alpha=0.9,
        )
        ax.plot(
            ...
            linewidth=3,
            markersize=8,
            label="Reconstructed Model → Reconstructed Test",
            color="red",
            alpha=0.9,
        )
        ...
        ax.set_title(
            f"Model B (Reconstructed Data): Subject-Level Probabilities\\n"
            f"{self.marker_type.title()} Features - Tested on Original vs Reconstructed Data\\n"
    ...
    def _plot_comparison_grid(self, results):
        ...
        ax.set_title(
            "Model A: Original Model\nTested on Original vs Reconstructed Data",
            fontsize=12,
            fontweight="bold",
        )
        ax.set_xticks(x)
        ...
        ax.set_ylabel("P(MCS)", fontsize=11, fontweight="bold")
        ax.set_title(
            "Original Test Data\nComparing Original vs Reconstructed Models",
            fontsize=12,
            fontweight="bold",
        )
        ax.set_xticks(x)
        ...
        # Overall title
        fig.suptitle(
            f"Probability Comparison Grid: All 4 Test Scenarios\n"
        ...
        prob_plot_file = op.join(
            self.output_dir, "subject_probabilities_comparison_grid.png"
        )
        plt.savefig(prob_plot_file, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def _plot_differences_same_train(self, results):
        ...
        diff_OO_OR = (
            proba_OO - proba_OR
        )  # Original model: how much higher on original vs reconstructed
        diff_RR_RO = (
            proba_RR - proba_RO
        )  # Reconstructed model: how much higher on reconstructed vs original
        ...
        ax.set_xlabel("Subject ID", fontsize=13, fontweight="bold")
        ax.set_ylabel(
            "Δ P(MCS) - Probability Difference", fontsize=13, fontweight="bold"
        )
        ax.set_title(
            f"Probability Differences: Same Training Model, Different Test Data\\n"
    ...
    def _plot_differences_same_test(self, results):
        ...
        diff_OO_RO = (
            proba_OO - proba_RO
        )  # Original test: how much higher with original vs reconstructed model
        diff_OR_RR = (
            proba_OR - proba_RR
        )  # Reconstructed test: how much higher with original vs reconstructed model
        ...
        ax.set_title(
            f"Probability Differences: Same Test Data, Different Training Model\\n"
    ...
    def _plot_probability_boxplots(self, results):
        ...
        data_to_plot.extend(
            [proba_OO[vs_mask], proba_OR[vs_mask], proba_RO[vs_mask], proba_RR[vs_mask]]
        )
        labels.extend(["OO", "OR", "RO", "RR"])
        colors.extend(["blue", "orange", "green", "red"])
        ...
        # Customize plot
        ax.set_ylabel(
            "P(MCS) - Probability of MCS Class", fontsize=14, fontweight="bold"
        )
        ax.set_xlabel("Subject Type", fontsize=14, fontweight="bold")
        ax.set_title(
            f"Probability Distribution by Model and Subject Type\n{self.marker_type.title()} Features",
            fontsize=15,
            fontweight="bold",
            pad=20,
        )
        ...
        for pos, label, color in zip(
            [1, 2, 3, 4], ["OO", "OR", "RO", "RR"], ["blue", "orange", "green", "red"]
        ):
            ax.text(
                pos,
                -0.08,
        ...
        for pos, label, color in zip(
            [6, 7, 8, 9], ["OO", "OR", "RO", "RR"], ["blue", "orange", "green", "red"]
        ):
            ax.text(
                pos,
                -0.08,
        ...
        ax.axvline(
            x=5, color="black", linestyle="-", linewidth=1, alpha=0.3
        )  # Separator
        ...
        legend_elements = [
            Patch(
                facecolor="blue", alpha=0.6, label="OO: Original Model → Original Test"
            ),
            Patch(
                facecolor="orange",
                alpha=0.6,
        ...
        ax.legend(
            handles=legend_elements, loc="upper right", fontsize=10, framealpha=0.95
        )

        plt.tight_layout()
        prob_boxplot_file = op.join(
            self.output_dir, "probability_boxplots_by_model_and_subject.png"
        )
    ...
    def _plot_difference_boxplots_with_stats(self, results):
        ...
        for i, (diff, name) in enumerate(zip(differences, diff_names)):
            if len(diff) > 0:
                try:
                    ...
                    print(
                        f"      - median diff={median_diff:.4f}, mean diff={mean_diff:.4f}, std diff={std_diff:.4f}"
                    )
        ...
        plt.tight_layout()
        diff_boxplot_file = op.join(
            self.output_dir, "difference_boxplots_with_wilcoxon.png"
        )
        ...
        self._plot_difference_boxplots_with_stats_small(
            results, differences, p_values, colors
        )
    ...
    def _plot_difference_boxplots_with_stats_small(
        self, results, differences, p_values, colors
    ):
        ...
        plt.tight_layout()
        diff_boxplot_small_file = op.join(
            self.output_dir, "difference_boxplots_with_wilcoxon_small.png"
        )
    ...
    def _plot_scenario_results(
        self, scenario_results, output_dir, scenario_key, global_results
    ):
        ...
        model_name = (
            "Model A (Original)"
            if "model_A" in scenario_key
            else "Model B (Reconstructed)"
        )
        test_name = (
            "Original Test" if "orig_test" in scenario_key else "Reconstructed Test"
        )

        fig.suptitle(
            f"Cross-Data Classification Results\n"
        ...
        if "model_A" in scenario_key:
            ...
            cv_label = (
                "Model B: Grid Search Best CV Score\n(Reconstructed Training Data)"
            )
        ...
        if importances is not None:
            ...
            if self.feature_names_abbreviated is not None and len(
                ...
                feature_labels = [
                    self.feature_names_abbreviated[i] for i in top_indices
                ]
            else:
                feature_labels = [f"Feature {i}" for i in top_indices]
            ...
            ax.text(
                0.5,
                0.5,
                f"Feature importances not available\n(using {kernel_used.upper()} kernel)",
                ha="center",
                va="center",
                transform=ax.transAxes,
                fontsize=12,
        ...
        if scenario_results["auc_score"] is not None:
            self._plot_roc_curve(
                scenario_results, output_dir, test_name, global_results["y_test"]
            )
    ...
    def _plot_combined_confusion_matrices(self, results):
        ...
        fig.suptitle(
            f"Cross-Data Classification: Confusion Matrices\n{self.marker_type.title()} Features",
            fontsize=18,
            fontweight="bold",
        )
        ...
        for scenario_key, row, col, title in scenarios:
            ...
            # Plot heatmap
            im = ax.imshow(
                conf_matrix, interpolation="nearest", cmap="Blues", aspect="auto"
            )
            ...
            if abs(calculated_accuracy - accuracy) > 0.001:
                print(
                    f"⚠️  WARNING: Accuracy mismatch! Computed {accuracy:.3f}, CM suggests {calculated_accuracy:.3f}"
                )

            ax.text(
                0.98,
                0.10,
                f"CM: {conf_matrix.tolist()}",
                transform=ax.transAxes,
                fontsize=8,
                fontweight="bold",
            ...
            ax.text(
                0.98,
                0.06,
                f"Raw Accuracy: {accuracy:.3f} ({n_correct}/{n_total})",
                transform=ax.transAxes,
                fontsize=10,
                fontweight="bold",
                verticalalignment="bottom",
    ...
    def _plot_auc_heatmap(self, results):
        ...
        auc_values = [
            [
                results["model_A_orig_test"]["auc_score"],
                results["model_B_orig_test"]["auc_score"],
            ],  # Tested on Original (upper row)
            [
                results["model_A_recon_test"]["auc_score"],
                results["model_B_recon_test"]["auc_score"],
            ],  # Tested on Reconstructed (lower row)
        ]

        # Convert to numpy array and handle None values
        auc_array = np.array(auc_values)
        auc_array = np.where(
            auc_array == None, np.nan, auc_array
        ...
        for i in range(2):
            for j in range(2):
                ...
                if not np.isnan(auc_val):
                    ...
                    ax.text(
                        j, i, "N/A", ha="center", va="center", fontsize=14, color="red"
                    )
...
def main():
    ...
    parser.add_argument(
        "--data-dir",
        default="/data/project/eeg_foundation/src/doc_benchmark/results/new_results/MARKERS/computed_data",
    ...
    parser.add_argument(
        "--output-dir",
        default="/data/project/eeg_foundation/src/doc_benchmark/results/new_results/MODEL/svm_crs_metadata",
    ...
    parser.add_argument(
        "--n-splits", type=int, default=6, help="Number of CV splits (ignored for LOO)"
    )
    parser.add_argument(
