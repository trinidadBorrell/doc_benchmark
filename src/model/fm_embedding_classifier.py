...
::
    ...
    python fm_embedding_classifier.py \\
        --data-dir /path/to/embeddings \\
        --patient-labels /path/to/patient_labels_with_controls.csv \\
        --output-dir /path/to/out

    # 5-fold nested CV
    python fm_embedding_classifier.py ... --full-cv --n-cv-folds 5
...
class EmbeddingClassifier:
    ...
    def __init__(
        ...
        self._baseline_states_dict = None  # set by load_patient_labels_for_target
    ...
    def _apply_train_pca(self, X_train, X_test):
        """Fit PCA on X_train and transform both X_train and X_test.

        No leakage: the projection is fit on the training fold only.
        Components are clamped to ``min(self.pca_components, n_train-1, n_features)``.
        Returns (X_train_t, X_test_t, n_components, explained_variance).
        """
        n_train = X_train.shape[0]
        n_feat = X_train.shape[1]
        n_comp = int(min(self.pca_components, max(n_train - 1, 1), n_feat))
    ...
    def run_classification(
        ...
        # PCA (train-only fit, no leakage). Applied once for the classic_split.
        pca_info = None
        if self.pca:
            X_trainval_in, X_test_in, n_comp, var_exp = self._apply_train_pca(
                X_trainval, X_test
            )
            pca_info = {"n_pca_components": n_comp, "pca_explained_variance": var_exp}
            print(
                f"   PCA(train-only): n_components={n_comp}, "
                f"explained_variance={var_exp:.4f}",
                flush=True,
            )
        else:
            X_trainval_in, X_test_in = X_trainval, X_test

        # Output goes under classic_split/ or classic_split_pca{N}/ when PCA is on.
        split_name = (
            f"classic_split_pca{self.pca_components}" if self.pca else "classic_split"
        )
        split_dir = op.join(self.output_dir, split_name)
        os.makedirs(split_dir, exist_ok=True)

        def _augment(res, best_params):
            res.update(
                {
                    "n_train": len(X_trainval),
                    "n_test": len(X_test),
                    "n_features": int(X_trainval_in.shape[1]),
        ...
        if "mlp" in self.classifiers:
            ...
            mlp_model, mlp_best_params = self._grid_search_mlp(
                X_trainval_in, y_trainval, groups_trainval
            )
            mlp_probs, mlp_preds = self._predict_mlp(mlp_model, X_test_in)
            mlp_results = self._compute_test_metrics(
                y_test, mlp_preds, mlp_probs, subjects_test
            )
            _augment(mlp_results, mlp_best_params)
            ...
            self._plot_results(
                ...
                n_features=X_trainval_in.shape[1],
        ...
        if "random_forest" in self.classifiers:
            ...
            rf_probs, rf_preds = self._predict_sklearn(rf_model, X_test_in)
            rf_results = self._compute_test_metrics(
                y_test, rf_preds, rf_probs, subjects_test
            )
            _augment(rf_results, rf_best_params)
        ...
        if "kernel_ridge" in self.classifiers:
            ...
            kr_probs, kr_preds = self._predict_sklearn(kr_model, X_test_in)
            kr_results = self._compute_test_metrics(
                y_test, kr_preds, kr_probs, subjects_test
            )
            _augment(kr_results, kr_best_params)
        ...
        if "svm" in self.classifiers:
            ...
            svm_probs, svm_preds = self._predict_sklearn(svm_model, X_test_in)
            svm_results = self._compute_test_metrics(
                y_test, svm_preds, svm_probs, subjects_test
            )
            _augment(svm_results, svm_best_params)
        ...
        # Summary
        print("\n" + "=" * 80, flush=True)
        print(f"EMBEDDING CLASSIFICATION SUMMARY ({split_name})", flush=True)
        print("=" * 80, flush=True)
    ...
    def run_full_cv(
        ...
        # Output goes under nested_cv/ or nested_cv_pca{N}/ when PCA is on.
        cv_name = (
            f"nested_cv_pca{self.pca_components}" if self.pca else "nested_cv"
        )
        cv_dir = op.join(self.output_dir, cv_name)
        os.makedirs(cv_dir, exist_ok=True)

        # Accumulators per active classifier only.
        accum = {
        ...
        # Per-fold PCA stats accumulator (populated only when --pca is on).
        pca_fold_stats = []
        ...
        for fold_idx, fold in enumerate(folds):
            ...
            # Per-fold PCA fit on train only (no leakage).
            if self.pca:
                X_train_in, X_test_in, n_comp, var_exp = self._apply_train_pca(
                    X_train_fold, X_test_fold
                )
                pca_fold_stats.append(
                    {
                        "fold": fold_idx,
                        "n_pca_components": n_comp,
                        "pca_explained_variance": var_exp,
                    }
                )
                print(
                    f"   PCA(train-only): n_components={n_comp}, "
                ...
                X_train_in, X_test_in = X_train_fold, X_test_fold
            ...
            if "mlp" in self.classifiers:
                ...
                mlp_fold_dir = op.join(cv_dir, "mlp")
            ...
            if "random_forest" in self.classifiers:
                ...
                rf_fold_dir = op.join(cv_dir, "random_forest")
            ...
            if "kernel_ridge" in self.classifiers:
                ...
                kr_fold_dir = op.join(cv_dir, "kernel_ridge")
            ...
            if "svm" in self.classifiers:
                ...
                svm_fold_dir = op.join(cv_dir, "svm")
        ...
        for model_key, cfg in MODEL_CONFIGS.items():
            ...
            if self.pca and pca_fold_stats:
                var_exps = [s["pca_explained_variance"] for s in pca_fold_stats]
                n_comps = [s["n_pca_components"] for s in pca_fold_stats]
                results["pca_per_fold"] = pca_fold_stats
                results["pca_explained_variance"] = float(np.mean(var_exps))
                results["pca_explained_variance_std"] = float(np.std(var_exps))
                results["n_pca_components"] = int(n_comps[0])
...
Reduction map for --marker-reduction:
  ...
  D = icm/lg/egi256gfp/std
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Apply train-only PCA to the FM embedding before classification. "
            "Fit per fold on the training set to avoid leakage. "
            "Output lands under a *_pca{N}/ split dir so PCA and non-PCA "
    ...
    parser.add_argument(
        ...
        default=27,
        help=(
            "Number of PCA components (default: 27 — matches the DK marker "
            "dimensionality after excluding TimeLockedContrast/WindowDecoding)."
        ),
    )
    parser.add_argument(
        "--batch-modes",
        ...
        for target in args.targets:
            ...
            for (
                ...
                if args.skip_common_pool:
                    common_sessions = None
                    precomputed_splits = None
                    print("\nSkipping common session pool building (--skip-common-pool).", flush=True)
                else:
                ...
                use_splits_file = (
                    not args.skip_common_pool
                    and args.splits_file
                ...
                elif not args.skip_common_pool:
