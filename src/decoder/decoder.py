...
def load_epochs_single_subject_session(
    main_path,
    subject_id,
    session_id,
    filter_trial_types=None,
    verbose=False,
    original_data_paths=None,
):
    """
    ...
    if original_data_paths:
        ...
        for original_data_path in original_data_paths:
            ...
            if search_dir.exists():
                ...
                found = list(
                    search_dir.glob(
                        f"sub-{subject_id}_ses-{session_id}_task-*_epo_original.fif"
                    )
                )
                if not found:
                    found = list(
                        search_dir.glob(
                            f"sub-{subject_id}_ses-{session_id}_task-*_epo.fif"
                        )
                    )
                if not found:
    ...
    if filter_trial_types:
        filter_event_ids = [
            event_id
            for event_id, name in event_id_mapping.items()
    ...
    if len(set(shapes)) > 1:
        print(
            f"  WARNING: Shape mismatch across epochs for sub-{subject_id} ses-{session_id}:"
        )
        for i, (ep, shape) in enumerate(zip(all_epochs, shapes)):
            label = (
                "original"
                if i < len([l for l in all_labels[: ep.shape[0]] if l == 0])
                else "reconstructed"
            )
...
def decode_single_subject_session(
    X, y, events, times, cv=10, n_jobs=None, mode="patient", verbose=False
):
    """
    ...
    if X is None or len(np.unique(y)) < 2:
        if X is None:
            print("  No data loaded (X is None)")
        else:
            unique_labels = np.unique(y)
            print(
                f"  Cannot decode: only {len(unique_labels)} class(es) found (labels={unique_labels})"
            )
            print(
                f"  Original epochs (y=0): {np.sum(y == 0)}, Reconstructed epochs (y=1): {np.sum(y == 1)}"
            )
        return None

    if X.shape[0] < cv:
        print(
            f"  WARNING: Too few trials ({X.shape[0]}) for {cv}-fold CV, need at least {cv}"
        )
        return None
        ...
        for trial_type, event_id in zip(target_trial_types, target_event_ids):
            ...
            try:
                ...
                scores = cross_val_multiscore(
                    time_decod, X_trial, y_trial, cv=cv, n_jobs=n_jobs, verbose=1
                )
                ...
                print(
                    f"    {trial_type} AUC: {results['trial_types'][trial_type]['mean_auc']:.3f}"
                )
            except Exception as e:
...
def save_single_subject_session_results(
    results, times, subject_id, session_id, output_dir
):
    ...
    if "overall" in results:
        ...
        if len(times) != n_scores:
            print(
                f"  WARNING: times length ({len(times)}) != scores length ({n_scores}) "
...
def run_permutation_test(all_scores, n_permutations=1000, seed=42):
    ...
    mean_auc_per_subject = np.mean(
        all_scores, axis=1
    )  # Average across timepoints for each subject
...
def create_single_subject_plots(results, times, plots_dir, subject_id, session_id):
    ...
    if "overall" in results:
        ...
        if len(times) != len(mean_scores):
            print(
                f"  WARNING: times ({len(times)}) and scores ({len(mean_scores)}) "
        ...
        plt.tight_layout()
        plt.savefig(
            plots_dir / "overall_classification.png", dpi=300, bbox_inches="tight"
        )
        plt.close()

    # Trial type plots (only if task events are present)
    if (
        "trial_types" in results
        and results["trial_types"]
        and results.get("has_task_events", False)
    ):
        ...
        plt.tight_layout()
        plt.savefig(
            plots_dir / "trial_type_classification.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def _truncate_to_common_length(timeseries_list):
    ...
    if min_len != max(lengths):
        print(
            f"  NOTE: Timeseries lengths vary ({min(lengths)}-{max(lengths)}), "
...
def aggregate_all_results(output_dir, patient_labels_path=None, mode="patient"):
    ...
    for i, results in enumerate(all_results):
        if "overall" in results:
            aggregated["overall"]["all_mean_aucs"].append(
                results["overall"]["mean_auc"]
            )
            ...
            aggregated["overall"]["all_mean_scores_time_dict"][key] = results[
                "overall"
            ]["mean_scores_time"]
    ...
    if aggregated["overall"]["all_mean_aucs"]:
        aggregated["overall"]["mean_auc"] = np.mean(
            aggregated["overall"]["all_mean_aucs"]
        )
        aggregated["overall"]["std_auc"] = np.std(
            aggregated["overall"]["all_mean_aucs"]
        )

        # Point-by-point statistics across subjects/sessions
        all_timeseries = _truncate_to_common_length(
        ...
        aggregated["overall"]["mean_scores_time"] = np.mean(
            all_timeseries, axis=0
        )  # (n_timepoints,)
    ...
    for trial_type in trial_types:
            ...
            if "trial_types" in results and trial_type in results["trial_types"]:
                ...
                if subject_state_map:
                    ...
                    if state in ["VS", "UWS"]:
                        aggregated["trial_types"][trial_type][
                            "all_mean_scores_time_VS"
                        ].append(results["trial_types"][trial_type]["mean_scores_time"])
                    elif state in ["MCS", "MCS-", "MCS+"]:
                        aggregated["trial_types"][trial_type][
                            "all_mean_scores_time_MCS"
                        ].append(results["trial_types"][trial_type]["mean_scores_time"])
        ...
        if aggregated["trial_types"][trial_type]["all_mean_scores_time"]:
            ...
            timeseries_dir.mkdir(exist_ok=True)
            np.save(
                timeseries_dir / f"all_mean_scores_time_{trial_type}.npy",
        ...
        if aggregated["trial_types"][trial_type]["all_mean_aucs"]:
            ...
            aggregated["trial_types"][trial_type]["mean_scores_time"] = np.mean(
                all_timeseries, axis=0
            )
            # Calculate std per timepoint (axis=0 gives std across subjects at each time)
            ...
            aggregated["trial_types"][trial_type]["sem_scores_time"] = aggregated[
                "trial_types"
            ][trial_type]["std_scores_time"] / np.sqrt(len(all_timeseries))
            aggregated["trial_types"][trial_type]["n_subjects_sessions"] = len(
                all_timeseries
            )
            ...
            if aggregated["trial_types"][trial_type]["all_mean_scores_time_VS"]:
                all_timeseries_VS = _truncate_to_common_length(
                    aggregated["trial_types"][trial_type]["all_mean_scores_time_VS"]
                )
                print(f"  {trial_type} VS timeseries shape: {all_timeseries_VS.shape}")
                aggregated["trial_types"][trial_type]["mean_scores_time_VS"] = np.mean(
                    all_timeseries_VS, axis=0
                )
                aggregated["trial_types"][trial_type]["std_scores_time_VS"] = np.std(
                    all_timeseries_VS, axis=0
                )
                aggregated["trial_types"][trial_type]["n_subjects_VS"] = len(
                    all_timeseries_VS
                )

            if aggregated["trial_types"][trial_type]["all_mean_scores_time_MCS"]:
                all_timeseries_MCS = _truncate_to_common_length(
                    aggregated["trial_types"][trial_type]["all_mean_scores_time_MCS"]
                )
                print(
                    f"  {trial_type} MCS timeseries shape: {all_timeseries_MCS.shape}"
                )
                aggregated["trial_types"][trial_type]["mean_scores_time_MCS"] = np.mean(
                    all_timeseries_MCS, axis=0
                )
                aggregated["trial_types"][trial_type]["std_scores_time_MCS"] = np.std(
                    all_timeseries_MCS, axis=0
                )
                aggregated["trial_types"][trial_type]["n_subjects_MCS"] = len(
                    all_timeseries_MCS
                )
        ...
        try:
            ...
            for state in target_groups:
                ...
                if len(state_res_list) < 2:  # Skip if too few subjects
                    print(
                        f"    Skipping {state}: only {len(state_res_list)} subject-sessions (need >=2)"
                    )
                    continue
                ...
                for res in state_res_list:
                    if "overall" in res:
                        aggregated["by_subject_type"][state]["overall"][
                            "all_mean_aucs"
                        ].append(res["overall"]["mean_auc"])
                        aggregated["by_subject_type"][state]["overall"][
                            "all_mean_scores_time"
                        ].append(res["overall"]["mean_scores_time"])

                # Calculate statistics
                if aggregated["by_subject_type"][state]["overall"]["all_mean_aucs"]:
                    all_aucs = aggregated["by_subject_type"][state]["overall"][
                        "all_mean_aucs"
                    ]
                    ...
                    aggregated["by_subject_type"][state]["overall"]["mean_auc"] = (
                        np.mean(all_aucs)
                    )
                    aggregated["by_subject_type"][state]["overall"]["std_auc"] = np.std(
                        all_aucs
                    )
                    aggregated["by_subject_type"][state]["overall"][
                        "mean_scores_time"
                    ] = np.mean(all_ts, axis=0)
                    aggregated["by_subject_type"][state]["overall"][
                        "std_scores_time"
                    ] = np.std(all_ts, axis=0)  # SD per time point
                    aggregated["by_subject_type"][state]["overall"]["n_subjects"] = len(
                        all_aucs
                    )

            print(
                f"  Aggregated by subject type: {list(aggregated['by_subject_type'].keys())}"
            )
        except Exception as e:
...
def save_aggregated_results(
    aggregated_results, times, subjects_sessions, output_dir, mode="patient"
):
    ...
    if "overall" in aggregated_results:
        if "all_mean_scores_time_dict" in aggregated_results["overall"]:
            np.save(
                data_dir / "all_mean_scores_time_dict.npy",
                aggregated_results["overall"]["all_mean_scores_time_dict"],
                allow_pickle=True,
            )

        if (
            "all_mean_scores_time_VS" in aggregated_results["overall"]
            and aggregated_results["overall"]["all_mean_scores_time_VS"]
        ):
            np.save(
                data_dir / "all_mean_scores_time_VS.npy",
                _truncate_to_common_length(
                    aggregated_results["overall"]["all_mean_scores_time_VS"]
                ),
            )
            print(
                f"Saved VS timeseries: {len(aggregated_results['overall']['all_mean_scores_time_VS'])} subjects"
            )

        if (
            "all_mean_scores_time_MCS" in aggregated_results["overall"]
            and aggregated_results["overall"]["all_mean_scores_time_MCS"]
        ):
            np.save(
                data_dir / "all_mean_scores_time_MCS.npy",
                _truncate_to_common_length(
                    aggregated_results["overall"]["all_mean_scores_time_MCS"]
                ),
            )
            print(
                f"Saved MCS timeseries: {len(aggregated_results['overall']['all_mean_scores_time_MCS'])} subjects"
            )

        if (
            "VS" in aggregated_results["overall"]
            and aggregated_results["overall"]["VS"]
        ):
            np.save(data_dir / "VS.npy", np.array(aggregated_results["overall"]["VS"]))
            print(
                f"Saved VS timeseries: {len(aggregated_results['overall']['VS'])} subjects"
            )

        if (
            "MCS" in aggregated_results["overall"]
            and aggregated_results["overall"]["MCS"]
        ):
            np.save(
                data_dir / "MCS.npy", np.array(aggregated_results["overall"]["MCS"])
            )
            print(
                f"Saved MCS timeseries: {len(aggregated_results['overall']['MCS'])} subjects"
            )
    ...
    for trial_type in trial_types:
        if trial_type in aggregated_results["trial_types"]:
            ...
            # Save VS + trial_type
            if (
                "all_mean_scores_time_VS" in trial_data
                and trial_data["all_mean_scores_time_VS"]
            ):
                np.save(
                    data_dir / f"all_mean_scores_time_VS_{trial_type}.npy",
                    _truncate_to_common_length(trial_data["all_mean_scores_time_VS"]),
                )
                print(
                    f"Saved VS_{trial_type} timeseries: {len(trial_data['all_mean_scores_time_VS'])} subjects"
                )

            # Save MCS + trial_type
            if (
                "all_mean_scores_time_MCS" in trial_data
                and trial_data["all_mean_scores_time_MCS"]
            ):
                np.save(
                    data_dir / f"all_mean_scores_time_MCS_{trial_type}.npy",
                    _truncate_to_common_length(trial_data["all_mean_scores_time_MCS"]),
                )
                print(
                    f"Saved MCS_{trial_type} timeseries: {len(trial_data['all_mean_scores_time_MCS'])} subjects"
                )
    ...
    for trial_type in ["LSGS", "LSGD", "LDGD", "LDGS"]:
        if (
            trial_type in aggregated_results["trial_types"]
            and "permutation_test" in aggregated_results["trial_types"][trial_type]
        ):
            perm_test = aggregated_results["trial_types"][trial_type][
                "permutation_test"
            ]
    ...
    print(
        f"Permutation test results saved to: {data_dir / 'permutation_test_results.json'}"
    )
    ...
    json_results = {
        ...
        "subjects_sessions": [
            {"subject_id": sub, "session_id": ses} for sub, ses in subjects_sessions
        ],
        "model_info": {
    ...
    if "overall" in aggregated_results and "mean_auc" in aggregated_results["overall"]:
        json_results["overall_classification"] = {
            ...
            "individual_mean_aucs": [
                float(x) for x in aggregated_results["overall"]["all_mean_aucs"]
            ],
        }
    ...
    for trial_type in ["LSGS", "LSGD", "LDGD", "LDGS"]:
        if (
            trial_type in aggregated_results["trial_types"]
            and "mean_auc" in aggregated_results["trial_types"][trial_type]
        ):
            trial_data = aggregated_results["trial_types"][trial_type]
...
def create_aggregated_plots(aggregated_results, times, plots_dir):
    ...
    # Overall classification plot
    if (
        "overall" in aggregated_results
        and "mean_scores_time" in aggregated_results["overall"]
    ):
        ...
        ax.plot(
            ...
            "b-",
            linewidth=3,
            label=f"Mean AUC (n={n_sessions} subjects)",
        )
        ax.fill_between(
            times,
        ...
        ax.text(
            0.02,
            0.98,
            f"Peak: AUC={peak_auc:.3f} at t={peak_time:.3f}s",
    ...
    ax.fill_between(
        times_plot,
        mean_scores_plot - std_scores_plot,
        mean_scores_plot + std_scores_plot,
        alpha=0.3,
        color="blue",
    )
    ...
    for i, trial_type in enumerate(trial_types):
        if (
            trial_type in aggregated_results["trial_types"]
            and "mean_scores_time" in aggregated_results["trial_types"][trial_type]
        ):
    ...
    for i, trial_type in enumerate(trial_types):
        if (
            ...
            ax.plot(
                times, mean_scores, color=colors[i], linewidth=2.5, label=trial_type
            )
            ax.fill_between(
                times,
                mean_scores - std_scores,
    ...
    if (
        "by_subject_type" in aggregated_results
        and aggregated_results["by_subject_type"]
    ):
        ...
        available_types = [
            ...
            if st in aggregated_results["by_subject_type"]
            and "overall" in aggregated_results["by_subject_type"][st]
            and "mean_scores_time"
            in aggregated_results["by_subject_type"][st]["overall"]
        ]
        ...
        if available_types:
            ...
            plt.suptitle(
                "Decoder Performance by Subject Type (Original vs Reconstructed EEG)\n"
                + "Groups: COMA | UWS (VS+UWS) | MCS (MCS-/MCS/MCS+)",
                fontsize=22,
            )
            plt.tight_layout()
            plt.savefig(
                plots_dir / "aggregated_by_subject_type.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
            ...
            print(
                f"  Created subject-type plot with {len(available_types)} types: {', '.join(available_types)}"
            )
...
def save_text_summary(aggregated_results, subjects_sessions, data_dir):
    ...
    with open(summary_path, "w") as f:
        ...
        if (
            "overall" in aggregated_results
            and "mean_auc" in aggregated_results["overall"]
        ):
            f.write("OVERALL CLASSIFICATION RESULTS\n")
            f.write("-" * 30 + "\n")
            f.write(
                f"Mean AUC (across sessions): {aggregated_results['overall']['mean_auc']:.6f}\n"
            )
            f.write(
                f"Std AUC (across sessions): {aggregated_results['overall']['std_auc']:.6f}\n\n"
            )
...
def main():
    ...
    if args.aggregate_only:
        ...
        aggregated_results, times, subjects_sessions = aggregate_all_results(
            args.output_dir, args.patient_labels, args.mode
        )
        ...
        global_dir = save_aggregated_results(
            aggregated_results, times, subjects_sessions, args.output_dir, args.mode
        )
        ...
        if (
            ...
            print("\nAGGREGATED SUMMARY:")
            print(
                f"  Overall AUC: {aggregated_results['overall']['mean_auc']:.3f} ± {aggregated_results['overall']['std_auc']:.3f}"
            )

            if aggregated_results["trial_types"]:
                print("  Trial Type AUCs:")
                for trial_type in ["LSGS", "LSGD", "LDGD", "LDGS"]:
                    if (
                        trial_type in aggregated_results["trial_types"]
                        and "mean_auc" in aggregated_results["trial_types"][trial_type]
                        ...
                        data = aggregated_results["trial_types"][trial_type]
                        print(
                            f"    {trial_type}: {data['mean_auc']:.3f} ± {data['std_auc']:.3f} (n={data['n_subjects_sessions']})"
                        )

            print(f"\nTotal subjects-sessions analyzed: {len(subjects_sessions)}")
            print(
                f"Subject-session combinations: {[f'sub-{s} ses-{ses}' for s, ses in subjects_sessions]}"
            )
    ...
    for subject_id in subjects:
        ...
        for session_id in available_sessions:
            ...
            if is_decoder_result_complete(args.output_dir, subject_id, session_id):
                print(
                    f"  ⏭️  Skipping sub-{subject_id} ses-{session_id} - results already exist"
                )
                skipped_count += 1
