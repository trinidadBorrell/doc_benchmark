...
def plot_global_with_pvalues(data_dir, output_dir, mode="DOC", analysis_type="overall"):
    ...
    if times_file.exists():
        ...
        all_scores = np.load(
            data_dir.parent / "all_auc_timeseries" / "all_mean_scores_time.npy"
        )
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / f"{filename_prefix}_global_with_pvalues.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def plot_all_global_analyses(results_dir, mode="DOC"):
    ...
    output_base = Path(
        "/data/project/eeg_foundation/src/doc_benchmark/results/new_results/DECODER/global_analysis"
    )
    ...
    if aggregated_results.get("has_task_events", False):
        ...
        for trial_type in trial_types:
            if trial_type in aggregated_results["trial_types"]:
                print(f"  - {trial_type}")
                plot_global_with_pvalues(
                    data_dir, output_dir, mode=mode, analysis_type=trial_type
                )
    else:
    ...
    with open(summary_file, "w") as f:
        f.write("DECODER GLOBAL ANALYSIS SUMMARY\n")
        f.write(f"={'=' * 50}\n\n")
        f.write(f"Mode: {mode}\n")
        ...
        if "overall" in aggregated_results:
            ...
            f.write(
                f"  N subjects: {len(aggregated_results['overall']['all_mean_scores_time'])}\n"
            )
        ...
        if aggregated_results.get("has_task_events", False):
            ...
            for trial_type in ["LSGS", "LSGD", "LDGD", "LDGS"]:
                if trial_type in aggregated_results["trial_types"]:
                    ...
                    # Load and report Wilcoxon results
                    wilcoxon_file = (
                        data_dir / f"{trial_type}_wilcoxon_per_timepoint.csv"
                    )
                    if wilcoxon_file.exists():
                        wilcoxon_df = pd.read_csv(wilcoxon_file)
                        n_sig = np.sum(wilcoxon_df["significant"])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create global plots with p-value highlighting"
    )
    parser.add_argument(
        "--results-dir",
        type=str,
