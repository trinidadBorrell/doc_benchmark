...
def _truncate_to_common_length(timeseries_list):
    ...
    if min(lengths) != max(lengths):
        print(
            f"  [truncation] Timeseries lengths vary ({min(lengths)}-{max(lengths)}), "
...
def analyze_decoder_results(results_dir, output_dir=None):
    ...
    print(f"\n{'=' * 60}")
    print("STATISTICAL ANALYSIS OF DECODER RESULTS")
    print(f"{'=' * 60}")
    ...
    analysis_results = {"overall": None, "trial_types": {}, "files_created": []}

    # Analyze overall classification
    if (
        "overall" in aggregated_results
        and "all_mean_scores_time" in aggregated_results["overall"]
    ):
        print("Analyzing overall classification...")
        all_scores = _truncate_to_common_length(
            aggregated_results["overall"]["all_mean_scores_time"]
        )
        ...
        print(
            f"  Significant timepoints: {n_sig}/{n_total} ({100 * n_sig / n_total:.1f}%)"
        )
        ...
        wilcoxon_df = pd.DataFrame(
            {
                ...
                else np.arange(len(wilcoxon_result["p_values"])),
    ...
    if has_task_events and "trial_types" in aggregated_results:
        ...
        for trial_type in trial_types:
            ...
            if all_scores.ndim < 2 or all_scores.shape[0] < 2:
                print(
                    f"    Skipping {trial_type}: insufficient data (need ≥2 subjects with 2D scores)"
                )
                continue

            # Run Wilcoxon test
            wilcoxon_result = run_wilcoxon_per_timepoint(
                all_scores, alternative="greater"
            )
    ...
    summary = {
        ...
        "overall": {
            "n_subjects": analysis_results["overall"]["n_subjects"]
            if analysis_results["overall"]
            else 0,
            "n_timepoints": analysis_results["overall"]["n_timepoints"]
            if analysis_results["overall"]
            else 0,
            "n_significant": int(
                np.sum(analysis_results["overall"]["significant_mask"])
            )
            if analysis_results["overall"]
            else 0,
        },
