...
def add_stimulus_lines(ax, times):
    ...
    for i, stim_time in enumerate(stimulus_times):
        if times[0] <= stim_time <= times[-1]:  # Only plot if within time range
            ...
            ax.axvline(
                stim_time, color="darkgray", linestyle="--", alpha=1, linewidth=1.5
            )
...
def group_by_diagnostic_group(subjects_sessions, patient_labels_path):
    ...
    print("  Diagnostic group distribution:")
...
def create_enhanced_ohbm_plot(
    ...
    for group in target_groups:
        if len(grouped_results[group]) >= 2:  # Need at least 2 subjects
            all_scores = np.array(
                [res["mean_scores_time"] for res in grouped_results[group]]
            )
            all_aucs = [res["mean_auc"] for res in grouped_results[group]]

            # Select all elements except the last 20 time points
            all_scores_trimmed = (
                all_scores[:, :-10] if all_scores.shape[1] > 10 else all_scores
            )
            ...
            print(
                f"  {group}: {len(grouped_results[group])} subjects, "
                f"Mean AUC = {group_stats[group]['mean_auc']:.3f} ± {group_stats[group]['std_auc']:.3f}"
            )
        else:
            print(
                f"  {group}: insufficient data ({len(grouped_results[group])} subjects)"
            )
    ...
    for i, group in enumerate(available_groups):
        ...
        ax.plot(
            times_trimmed,
            stats["mean_scores_time"],
            color=blue_palette[group],
            linewidth=2.5,
...
def create_trial_type_grid_plot(
    all_results, subjects_sessions, times, subject_group_map, output_dir
):
    """
        ...
        if group in target_groups and "trial_types" in all_results[i]:
            for trial_type in trial_types:
                if trial_type in all_results[i]["trial_types"]:
                    grouped_results[group][trial_type].append(
                        all_results[i]["trial_types"][trial_type]
                    )
    ...
    for group in target_groups:
        ...
        for trial_type in trial_types:
            if len(grouped_results[group][trial_type]) >= 1:  # Need at least 1 subject
                all_scores = np.array(
                    [
                        res["mean_scores_time"]
                        for res in grouped_results[group][trial_type]
                    ]
                )
                all_aucs = [
                    res["mean_auc"] for res in grouped_results[group][trial_type]
                ]
                ...
                print(
                    f"  {group}-{trial_type}: {len(grouped_results[group][trial_type])} subjects, "
                    f"Mean AUC = {group_stats[group][trial_type]['mean_auc']:.3f} ± {group_stats[group][trial_type]['std_auc']:.3f}"
                )
            else:
                print(
                    f"  {group}-{trial_type}: insufficient data ({len(grouped_results[group][trial_type])} subjects)"
                )
    ...
    for row_idx, group in enumerate(target_groups):
        for col_idx, trial_type in enumerate(trial_types):
            ...
            if stats is not None:
                ...
                # Plot mean line with SD shading
                ax.plot(
                    times_trimmed, stats["mean_scores_time"], color=color, linewidth=2.5
                )
    ...
    plt.suptitle(
        "Decoder Performance by Diagnostic Group and Trial Type\n"
...
def create_aggregated_overall_comparison_plot(output_dir):
    ...
    for idx, dataset in enumerate(datasets):
        ...
        all_scores_trimmed = (
            all_scores_array[:, :-10]
            if all_scores_array.shape[1] > 10
            else all_scores_array
        )
        times_trimmed = times[:-10] if len(times) > 10 else times
...
def main():
    ...
    subject_group_map = group_by_diagnostic_group(
        subjects_sessions, patient_labels_path
    )
