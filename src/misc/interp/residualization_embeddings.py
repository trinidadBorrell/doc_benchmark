"""Marker residualization of FM embeddings and VS/MCS classification.

=================================================================
Residualization + Classification Pipeline
=================================================================
...
2. **Linear probing (Ridge)**: for each neurophysiological marker, train a
   Ridge regressor that predicts the marker scalar from the embedding.
   The probe weight vector ``w_m`` (shape: ``emb_dim``) defines the
   embedding direction most predictive of marker ``m``.

3. **Residualize**: for each embedding ``x``, subtract the projections
   onto every probe direction::

       x_res = x - sum_m  (x @ w_hat_m) * w_hat_m

   This removes marker-predictable variance from the embeddings while
   preserving the remaining structure.

4. **Classify VS vs MCS** on the residualized embeddings using nested
   StratifiedGroupKFold CV with four classifiers: KernelRidge, SVM,
   MLP, RandomForest.

5. **Save results**: ``results.json``, ``results.csv`` and
   ``auc_comparison.png`` (dot + error-bar per FM model, y-axis 0–1).

The same ``--random-state`` seed governs both probe inner CV and
classification outer/inner folds, making results comparable across FM
models.
...
::
    ...
    python residualization_embeddings.py \\
    ...
    # Single model
    python residualization_embeddings.py --model LaBram ...

    # Different reduction
    python residualization_embeddings.py --reduction B ...
"""
...
def load_markers(marker_csv, reduction="A"):
    ...
    if n_dropped:
        print(
            f"   Dropped {n_dropped} excluded markers "
            f"(TimeLockedContrast / WindowDecoding)",
            flush=True,
        )
...
class MarkerResidualizer(BaseEstimator, TransformerMixin):
    """Remove marker-predictable directions from FM embeddings.

    For each neurophysiological marker, a Ridge regression probe is trained
    to predict the marker scalar from the embedding.  The unit-normalised
    weight vector ``w_hat_m`` of each probe defines the embedding direction
    most predictive of marker ``m``.

    During ``transform``, the projections of each embedding onto all probe
    directions are subtracted::

        x_res = x_scaled - sum_m  (x_scaled @ w_hat_m) * w_hat_m

    Parameters
    ----------
    alpha : float
        Ridge regularisation strength used for the probes.
    ...
    def fit(self, X, Y, marker_names, groups=None):
        """Fit one Ridge probe per marker.
        ...
        groups : ndarray, shape (n,) or None
            Subject-level group IDs for inner GroupKFold.  If None, inner CV
            is skipped and ``self.alpha`` is used directly.
        """
        ...
        self._probe_directions = []  # list of unit weight vectors (emb_dim,)
        self._n_probes_fitted = 0

        for i, name in enumerate(marker_names):
            y = Y[:, i]
            valid = np.isfinite(y)
            n_valid = valid.sum()
            if n_valid < 5 or np.std(y[valid]) < 1e-12:
                continue
            ...
            if n_inner >= 2:
                ...
                for a in self.alpha_grid:
                    fold_scores = []
                    for tr, va in inner_cv.split(X_v, groups=g_v):
                        ridge = Ridge(alpha=a)
                        ridge.fit(X_v[tr], y_v[tr])
                        fold_scores.append(ridge.score(X_v[va], y_v[va]))
                    mean_score = float(np.mean(fold_scores))
                    if mean_score > best_score:
                        best_score = mean_score
                        best_alpha = a

            # Refit on all valid training samples
            ridge = Ridge(alpha=best_alpha)
            ridge.fit(X_v, y_v)

            w = ridge.coef_  # (emb_dim,)
            w_norm = np.linalg.norm(w)
            if w_norm < 1e-12:
                continue

            self._probe_directions.append(w / w_norm)
            self._n_probes_fitted += 1

        print(
            f"      [Residualizer] Fitted {self._n_probes_fitted}/{len(marker_names)}"
            " probe directions",
            flush=True,
        )
        return self
    ...
    def transform(self, X):
        """Remove probe directions from embeddings.
        ...
        X_res : ndarray, shape (n, emb_dim)
            Scaled and residualized embeddings.
        """
        X_scaled = self._scaler.transform(X)
        X_res = X_scaled.copy()
        for w_hat in self._probe_directions:
            proj = X_res @ w_hat  # (n,)
            X_res -= np.outer(proj, w_hat)
        return X_res
...
def _build_classifier(clf_name, random_state):
    """Return an untrained classifier instance."""
    if clf_name == "kernel_ridge":
        return KernelRidgeClassifier(alpha=1.0, kernel="rbf")
    if clf_name == "svm":
        return SVC(
            C=1.0,
            kernel="rbf",
...
def _inner_cv_classifier(clf_name, X_train, y_train, groups_train, random_state, inner_splits=None):
    """Run inner CV grid search and return the best fitted classifier.

    Uses pre-computed ``inner_splits`` (list of (train_idx, val_idx) relative to
    X_train) when provided, otherwise falls back to 3-fold GroupKFold.
    ...
    if inner_splits is not None:
        cv_iter = inner_splits  # list of (inner_train_idx, inner_val_idx)
    else:
...
def _safe_auc(y_true, y_score, y_pred, is_multiclass=False):
    ...
    For multiclass targets uses balanced_accuracy directly.
    """
    if is_multiclass:
...
def run_nested_cv(
    ...
    """Nested CV with residualization.

    Outer loop: pre-computed StratifiedGroupKFold folds (from ``precomputed_splits``)
    or freshly generated StratifiedGroupKFold when ``precomputed_splits`` is None.
    For each outer fold:
      1. Fit MarkerResidualizer on (X_train, Y_train).
      2. Residualize X_train and X_test.
      3. Inner loop: hyperparameter search for classifier using pre-computed
         inner splits when available, else 3-fold GroupKFold.
      4. Evaluate on X_test_res (AUC for binary, balanced accuracy for multiclass).
    ...
    is_multiclass : bool
        When True, uses balanced_accuracy_score as the metric.
    ...
    # Accumulate per-fold AUC for each classifier
    ...
    for fold_idx, fold in enumerate(folds_iter):
        ...
        residualizer = MarkerResidualizer()
        residualizer.fit(X_train_raw, Y_train, marker_names, groups=groups_train)

        # ── Step 2: residualize ────────────────────────────────────────
        X_train_res = residualizer.transform(X_train_raw)
        X_test_res = residualizer.transform(X_test_raw)

        # ── Optional Step 2b: train-only PCA on residualized embeddings ─
        pca_n_comp = None
        pca_var_exp = None
        if pca:
            ...
            pca_n_comp = int(
                min(pca_components, max(n_train - 1, 1), n_feat)
            )
            ...
            print(
                f"      PCA(train-only): n_components={pca_n_comp}, "
                f"explained_variance={pca_var_exp:.4f}",
                flush=True,
            )
        ...
        for clf_name in classifiers:
            try:
                clf = _inner_cv_classifier(
                    clf_name, X_train_res, y_train, groups_train, random_state,
            ...
            if pca:
                detail["n_pca_components"] = pca_n_comp
                detail["pca_explained_variance"] = pca_var_exp
            fold_details[clf_name].append(detail)
            print(
...
def plot_auc_comparison(
    ...
    is_multiclass : bool
        If True, labels y-axis as balanced accuracy.
    """
    ...
    ax.set_title(
        f"{title_suffix} — embedding marker residualization\n"
...
def plot_summary_heatmap(summary_data, output_path):
    ...
    summary_data : list of dict
        Each dict: ``{"title": str, "model": str, "clf": str, "mean_auc": float}``.
    output_path : str
    """
    targets_seen = []
    models_seen = []
    ...
    ax.set_title(
        "Embedding residualization — best-classifier mean score\n"
...
def run_model(
    ...
    """Run the full residualization + classification pipeline for one FM model and target.
    ...
    if precomputed_splits is not None and common_sessions_ref is not None:
        ...
        if missing:
            ...
            # Use the reference order so fold indices map directly to X rows
...
def save_results(
    ...
    json_payload = {
        "method": "embedding_residualization",
        "reduction": reduction,
        "n_folds": n_folds,
        "target": target,
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Residualize FM embeddings w.r.t. EEG markers, then classify. "
    ...
    parser.add_argument(
        "--output-dir",
        default="results/residualization",
        ...
        for model_name in models_to_run:
            ...
            if not _splits_saved and splits_used is not None:
                ...
                if not save_path:
                    ...
                    save_path = op.join(splits_dir, f"emb_splits_{tag}_{ts}.json")
