...
def main():
    ...
    if not data_path_original.exists():
        logger.error(
            f"❌ Original computed data directory not found: {data_path_original}"
        )
        logger.error("Please run compute_data.py first to generate the data.")
        sys.exit(1)
    if not data_path_recon.exists():
        logger.error(
            f"❌ Reconstructed computed data directory not found: {data_path_recon}"
        )
        logger.error("Please run compute_data.py first to generate the data.")
        sys.exit(1)
    ...
    try:
        ...
        logger.info(
            f"Generating report for subject {args.subject_id}, session {args.session}"
        )
        ...
        report = mne.Report(
            title=f"Task: Local-Global - subject: {args.subject_id} - COMPARISON: Original vs Reconstructed"
        )

        # === PREPROCESSING PLOTS ===
        # Skip preprocessing plots if no preprocessing metadata available
        preprocessing_info = report_data.get("metadata", {}).get(
            "preprocessing_info", {}
        )
        if preprocessing_info:
            ...
            logger.info(
                "Skipping preprocessing plots (no preprocessing metadata available)"
            )

        # === DIAGNOSTIC PLOTS ===
        logger.info("Adding diagnostic plots...")
        try:
            _add_diagnostic_plots(
                report, epochs, data_path_original, data_path_recon, output_dir
            )
        ...
        try:
            _add_connectivity_plots(
                report, data_path_original, data_path_recon, output_dir
            )
        except Exception as e:
        ...
        output_path = (
            output_dir
            / f"sub-{args.subject_id}_ses-{args.session}_report_comparison.html"
        )
...
def _add_preprocessing_plots(report, epochs, report_data, save_dir=None):
    ...
    eeg_bad_channels = [
        ch for ch in real_bad_channels if ch.startswith("E") and ch[1:].isdigit()
    ]
    ...
    if epochs is not None:
        ...
        try:
            ...
            bad_idx = [
                i for i, ch in enumerate(evoked.ch_names) if ch in eeg_bad_channels
            ]
            ...
            good_idx = [
                i for i, ch in enumerate(evoked.ch_names) if ch not in eeg_bad_channels
            ]
            if len(good_idx) > 0:
            ...
            sphere, outlines = prepare_layout(
                epochs.info.get("description", "egi/256"), info=epochs.info
            )
            ...
            mne.viz.plot_topomap(
                ...
                contours=0,
                cmap="gray",
                names=[" " * 10 + x for x in evoked.ch_names],
                sensors=False,
                axes=new_axes,
                outlines=outlines,
            ...
            if save_dir:
                ...
                fig.savefig(
                    Path(save_dir) / f"{plot_name}.png", dpi=150, bbox_inches="tight"
                )
                # Save data
                np.savez(
            ...
            logging.getLogger(__name__).warning(
                f"Could not create bad channels plot: {e}"
            )
    ...
    logger.info(
        f"[TIMING] Completed preprocessing plots in {time.time() - start_time:.2f}s"
    )


def _create_side_by_side_figure(
    fig_left, fig_right, title_left="ORIGINAL", title_right="RECONSTRUCTED"
):
...
def _create_vertical_stacked_figure(
    fig_top, fig_bottom, title_top="ORIGINAL", title_bottom="RECONSTRUCTED"
):
...
def _add_diagnostic_plots(
    report, epochs, data_path_original, data_path_recon, save_dir=None
):
    ...
    if gfp_path_original.exists() or gfp_path_recon.exists():
        ...
        if gfp_path_original.exists():
            plot_gfp(
                computed_data_path=gfp_path_original,
                colors=["b", "b", "r", "r"],
                linestyles=["-", "--", "-", "--"],
                ax=axes[0],
        ...
        if gfp_path_recon.exists():
            plot_gfp(
                computed_data_path=gfp_path_recon,
                colors=["b", "b", "r", "r"],
                linestyles=["-", "--", "-", "--"],
                ax=axes[1],
                sns_kwargs={"style": "darkgrid"},
        ...
        if save_dir:
            ...
            save_data = {}
            if gfp_data_orig:
                save_data["original_evokeds"] = [
                    ev.data for ev in gfp_data_orig.get("evokeds", [])
                ]
                save_data["original_times"] = (
                    gfp_data_orig.get("evokeds", [{}])[0].times
                    if gfp_data_orig.get("evokeds")
                    else []
                )
            if gfp_data_recon:
                save_data["reconstructed_evokeds"] = [
                    ev.data for ev in gfp_data_recon.get("evokeds", [])
                ]
                save_data["reconstructed_times"] = (
                    gfp_data_recon.get("evokeds", [{}])[0].times
                    if gfp_data_recon.get("evokeds")
                    else []
                )
        ...
        report.add_figure(
            fig, title="All Blocks: Global Field Power Comparison", section="Diagnostic"
        )
        plt.close(fig)
    logger.info(
        f"[TIMING] Completed diagnostic plots in {time.time() - start_time:.2f}s"
    )
...
def _add_cnv_plots(report, data_path_original, data_path_recon, save_dir=None):
    ...
    if cnv_path_original.exists() and cnv_path_recon.exists():
        ...
        logger.info("📊 CNV SCALE MATCHING:")
        ...
        if vminmax_orig is not None and vminmax_recon is not None:
            ...
            logger.warning("   ⚠️  Could not match scales - missing vminmax values")

        # Generate both plots
        fig_orig = plot_cnv(
            computed_data=data_orig,
            event_times=event_times,
            sns_kwargs={"style": "white"},
        )
        fig_recon = plot_cnv(
            computed_data=data_recon,
            event_times=event_times,
            sns_kwargs={"style": "white"},
        )
        ...
        if save_dir and fig_orig:
            ...
            np.savez(
                ...
                times=data_orig.get("evokeds", [{}])[0].times
                if data_orig.get("evokeds")
                else [],
                vminmax=data_orig.get("vminmax"),
            )
            logger.info("   💾 Saved: cnv_original")
        if save_dir and fig_recon:
            ...
            np.savez(
                ...
                times=data_recon.get("evokeds", [{}])[0].times
                if data_recon.get("evokeds")
                else [],
                vminmax=data_recon.get("vminmax"),
            )
            logger.info("   💾 Saved: cnv_reconstructed")
    else:
        # Load individual plots if only one exists
        if cnv_path_original.exists():
            fig_orig = plot_cnv(
                computed_data_path=cnv_path_original,
                event_times=event_times,
                sns_kwargs={"style": "white"},
            )
            if save_dir and fig_orig:
        ...
        if cnv_path_recon.exists():
            fig_recon = plot_cnv(
                computed_data_path=cnv_path_recon,
                event_times=event_times,
                sns_kwargs={"style": "white"},
            )
            if save_dir and fig_recon:
    ...
    if fig_orig or fig_recon:
        fig_combined = _create_vertical_stacked_figure(
            fig_orig,
            fig_recon,
        ...
        if save_dir:
            ...
            if data_orig:
                save_data["original_evokeds"] = [
                    ev.data for ev in data_orig.get("evokeds", [])
                ]
                save_data["original_times"] = (
                    data_orig.get("evokeds", [{}])[0].times
                    if data_orig.get("evokeds")
                    else []
                )
...
def _add_spectral_plots(report, data_path_original, data_path_recon, save_dir=None):
    ...
    def plot_with_matched_scales(
        path_orig, path_recon, title_base, section, plot_prefix
    ):
        ...
        if fig_orig or fig_recon:
            fig_combined = _create_side_by_side_figure(
                fig_orig,
                fig_recon,
            ...
            report.add_figure(
                fig_combined, title=f"{title_base} - Comparison", section=section
            )
            plt.close(fig_combined)
            if fig_orig:
                plt.close(fig_orig)
...
def _add_information_theory_plots(
    report, data_path_original, data_path_recon, save_dir=None
):
    ...
    logger.info(
        f"[TIMING] Completed information theory plots in {time.time() - start_time:.2f}s"
    )
...
def _add_erp_plots(
    ...
    figs_orig = _generate_erp_figures(
        epochs,
        skip_clustering,
        data_path_original,
        "ORIGINAL",
    ...
    figs_recon = _generate_erp_figures(
        epochs,
        skip_clustering,
        data_path_recon,
        "RECONSTRUCTED",
    ...
    for key in figs_orig.keys():
        ...
        if fig_orig or fig_recon:
            fig_combined = _create_side_by_side_figure(
                fig_orig, fig_recon, f"ORIGINAL: {key}", f"RECONSTRUCTED: {key}"
            )
...
def _generate_erp_figures(
    epochs, skip_clustering, data_path, label_prefix, save_dir=None, reference_info=None
):
    ...
    if local_gfp_path.exists():
        try:
            fig_gfp = plot_gfp(
                computed_data_path=local_gfp_path,
                colors=["r", "b"],
                labels=local_labels,
    ...
    if local_contrast_path.exists():
        try:
            evoked, _, _, local_contrast = get_contrast(
                computed_data_path=local_contrast_path
            )
            ...
            stat_cmap = get_stat_colormap(
                stat_logpsig, stat_vmin_actual, stat_vmax_actual
            )

            fig_topo_stat = plot_evoked_topomap(
                local_contrast.mlog10_p_val,
                times=plot_times,
                ch_type="eeg",
                contours=0,
                cmap=stat_cmap,
    ...
    if not skip_clustering:
        ...
        if local_cluster_path.exists():
            try:
                fig_cluster = plot_cluster_test(
                    computed_data_path=local_cluster_path,
                    sns_kwargs={"style": "darkgrid"},
                )
    ...
    if global_gfp_path.exists():
        try:
            fig_gfp = plot_gfp(
                computed_data_path=global_gfp_path,
                colors=["r", "b"],
    ...
    if global_contrast_path.exists():
        try:
            evoked, _, _, global_contrast = get_contrast(
                computed_data_path=global_contrast_path
            )
            _ensure_montage(evoked, reference_info)
            ...
            stat_cmap_global = get_stat_colormap(
                stat_logpsig, stat_vmin_global, stat_vmax_global
            )

            fig_topo_stat = plot_evoked_topomap(
                global_contrast.mlog10_p_val,
                times=plot_times,
                ch_type="eeg",
                contours=0,
                cmap=stat_cmap_global,
                scalings=1,
                cbar_fmt="%0.3f",
                average=0.04,
                units="-log10(p)",
    ...
    if not skip_clustering:
        ...
        if global_cluster_path.exists():
            try:
                fig_cluster = plot_cluster_test(
                    computed_data_path=global_cluster_path,
                    sns_kwargs={"style": "darkgrid"},
                )
...
def _add_prediction_plots(report, data_path_original, data_path_recon, save_dir=None):
    ...
    if prediction_path_recon.exists():
        ...
        _process_prediction_data(
            report, prediction_path_recon, "RECONSTRUCTED", save_dir
        )
    ...
    logger.info(
        f"[TIMING] Completed prediction plots in {time.time() - start_time:.2f}s"
    )
...
def _process_prediction_data(report, prediction_path, label_prefix, save_dir=None):
    ...
    try:
        ...
        report.add_html(
            html, title=f"{label_prefix}: Prediction Summary", section="Prediction"
        )
        ...
        for clf_type, results in multivariate.items():
            ...
            clf_display_name = (
                "Extra Trees" if clf_type == "et-reduced" else "Gaussian SVM"
            )
            ...
            report.add_figure(
                fig_overall,
                title=f"{label_prefix}: {clf_display_name}",
        ...
        if univariate is not None and not univariate.empty and len(univariate) > 0:
            ...
            logger.info(
                f"Creating univariate visualizations for {len(univariate)} markers"
            )

            # Violin plot
            stacked_df = univariate.set_index(["Marker", "Reduction"])[
                ["VS/UWS", "MCS"]
            ].stack()
            ...
            report.add_figure(
                fig_violin,
                title=f"{label_prefix}: Univariate Summary I",
            ...
            if grouped_data:
                ...
                for i, (group_name, group_df) in enumerate(grouped_data.items()):
                    ...
                    ax.set_ylim(
                        [-bar_height / 2, bar_height * n_markers - bar_height / 2]
                    )
                ...
                report.add_figure(
                    fig_bars,
                    title=f"{label_prefix}: Univariate Summary II",
                    section="Univariate Summaries",
                )
                plt.close(fig_bars)
