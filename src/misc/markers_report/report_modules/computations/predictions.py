...
class ICMPredictor:
    ...
    def __init__(self, config_params=None, models_dir=None):
        ...
        self.reductions = config_params.get(
            "reductions",
            [
        ...
        self.classifiers, self.feature_cols, self.training_metadata = (
        ...
        self.marker_groups = {
            ...
            "Connectivity": ["SymbolicMutualInformation/weighted"],
            "ERPs": [
    ...
    def _load_pretrained_models(self):
        ...
        logger.info(
            f"Loaded training metadata: {training_metadata['n_samples']} samples, {training_metadata['n_features']} features"
        )
        ...
        for clf_name in self.clf_types:
            ...
            if clf_metadata_path.exists():
                ...
                logger.info(
                    f"  CV AUC: {clf_metadata['cv_score_mean']:.3f} ± {clf_metadata['cv_score_std']:.3f}"
                )
                logger.info(
                    f"  In-sample accuracy: {clf_metadata['in_sample_score']:.3f}"
                )
    ...
    def predict(self, markers_data, summary=None):
        ...
        subject_features_df, subject_feature_names = (
            self._extract_features_with_reductions(markers_data)
        )
        ...
        logger.info(
            f"Extracted {len(subject_feature_names)} features from subject data"
        )

        # Prepare subject data (align features with pretrained model's expected features)
        X_subject = self._align_subject_features(
            subject_features_df, self.feature_cols, subject_feature_names
        )
        ...
        if self.univariate_models is not None:
            univariate_results = self._univariate_prediction_pretrained(
                X_subject, self.feature_cols
            )
        else:
            univariate_results = self._univariate_prediction_simple(
                X_subject, self.feature_cols
            )
    ...
    def _extract_features_with_reductions(self, markers_data):
        ...
        if "epoch_info" in markers_data:
            ...
            logger.warning(
                "No epoch_info found, will try to infer dimensions from data"
            )
        ...
        if "per_channel_spectral" in markers_data:
            ...
            for band in ["delta", "theta", "alpha", "beta", "gamma"]:
                if band in spectral_data:
                    ...
                    if is_linear_scale:
                        logger.info(
                            f"PSD/{band}: Converting from linear to dB scale (10*log10)"
                        )
                        band_data_db = 10 * np.log10(raw_data)
                        logger.info(
                            f"  Before: min={np.min(raw_data):.6f}, max={np.max(raw_data):.6f}"
                        )
                        logger.info(
                            f"  After:  min={np.min(band_data_db):.3f}, max={np.max(band_data_db):.3f}"
                        )
                    else:
                        logger.info(
                            f"PSD/{band}: Already in dB scale (has negative values or very small values)"
                        )
                ...
                if band_norm in normalized_data:
                    for reduction_type in reduction_types:
                        try:
                            ...
                            logger.debug(
                                f"Failed PSD/{band_norm}/{reduction_type}: {e}"
                            )
        ...
        if "spectral_entropy" in markers_data:
            for reduction_type in reduction_types:
                try:
                    ...
                    key = (
                        f"nice/marker/PowerSpectralDensity/summary_se_{reduction_type}"
                    )
                    features[key] = scalar_value
                except Exception as e:
        ...
        if "kolmogorov_complexity" in markers_data:
            for reduction_type in reduction_types:
                try:
                    scalar_value = reduce_to_scalar(
                        markers_data["kolmogorov_complexity"],
                        n_epochs,
                        n_channels,
        ...
        if (
            "cnv_data" in markers_data
            and "cnv_slopes_trials" in markers_data["cnv_data"]
        ):
            cnv_slopes = markers_data["cnv_data"]["cnv_slopes_trials"]
            for reduction_type in reduction_types:
                try:
                    config = get_lg_reduction_config(
                        reduction_type, "ContingentNegativeVariation"
                    )
                    ...
                    data_filtered = (
                        cnv_slopes[:, cnv_roi] if cnv_roi is not None else cnv_slopes
                    )
                ...
                if topo_data.ndim == 3 and topo_data.shape[1] == n_channels_topo:
                    ...
                    for reduction_type in reduction_types:
                        try:
                            ...
                            logger.warning(
                                f"Failed TimeLockedTopography/{topo_type}/{reduction_type}: {e}"
                            )
                else:
                    logger.warning(
                        f"Unexpected shape for {topo_key}: {topo_data.shape}, expected (epochs, {n_channels_topo}, times)"
                    )
        ...
        contrast_mapping = {
            "timelockedcontrast_lsgs_ldgd_timelockedcontrast": (
                "LSGS-LDGD",
                epoch_counts.get("LSGS", 0) + epoch_counts.get("LDGD", 0),
            ),
            "timelockedcontrast_lsgd_ldgs_timelockedcontrast": (
                "LSGD-LDGS",
                epoch_counts.get("LSGD", 0) + epoch_counts.get("LDGS", 0),
            ),
            "timelockedcontrast_ld_ls_timelockedcontrast": (
                "LD-LS",
                epoch_counts.get("LDGS", 0)
                + epoch_counts.get("LDGD", 0)
                + epoch_counts.get("LSGS", 0)
                + epoch_counts.get("LSGD", 0),
            ),
        ...
        for contrast_key, (
            ...
            if contrast_key in markers_data:
                ...
                if contrast_data.ndim == 3 and contrast_data.shape[1] == n_channels:
                    ...
                    contrast_time_avg = np.mean(
                        contrast_data, axis=2
                    )  # -> (combined_epochs, channels)
                    ...
                    for reduction_type in reduction_types:
                        try:
                            marker_type = (
                                f"TimeLockedContrast/{contrast_name}"
                                if contrast_name in ["mmn", "p3a", "p3b"]
                                else "TimeLockedContrast"
                            )
                            ...
                            logger.warning(
                                f"Failed TimeLockedContrast/{contrast_name}/{reduction_type}: {e}"
                            )
                else:
                    logger.warning(
                        f"Unexpected shape for {contrast_key}: {contrast_data.shape}, expected (epochs, {n_channels}, times)"
                    )
        ...
        if features:
            ...
            logger.info(
                f"Extracted {len(feature_names)} features using NICE reductions"
            )
    ...
    def _align_subject_features(
        self, subject_features_df, training_feature_cols, subject_feature_names
    ):
        """
        ...
        subject_feature_map = {
            name: idx for idx, name in enumerate(subject_feature_names)
        }
        ...
        if matched_features < len(training_feature_cols) * 0.5:
            logger.warning(
                f"Only matched {matched_features}/{len(training_feature_cols)} features between subject and training data"
            )
            if len(unmatched_features) <= 10:
                logger.warning(f"Unmatched training features: {unmatched_features}")
            else:
                logger.warning(
                    f"Unmatched training features (first 10): {unmatched_features[:10]}"
                )
            # Log first few subject features for debugging
            logger.warning(
                f"Subject feature examples (first 5): {subject_feature_names[:5]}"
            )
        else:
            logger.info(
                f"Matched {matched_features}/{len(training_feature_cols)} features"
            )
    ...
    def _multivariate_prediction_pretrained(self, X_subject):
        ...
        for clf_name, clf in self.classifiers.items():
            ...
            try:
                ...
                clf_metadata_path = self.models_dir / f"{clf_name}_metadata.joblib"
                clf_metadata = (
                    joblib.load(clf_metadata_path) if clf_metadata_path.exists() else {}
                )
                ...
                logger.info(
                    f"    Predicted: {self.ml_classes[0]}={probas[0, 0]:.3f}, {self.ml_classes[1]}={probas[0, 1]:.3f}"
                )
                if feature_importance is not None:
                    logger.info(
                        f"    Extracted feature importance for {len(feature_importance)} features"
                    )
    ...
    def _univariate_prediction_pretrained(self, X_subject, feature_cols):
            ...
            try:
                ...
                if len(parts) >= 2:
                    ...
                    for i in range(len(parts) - 1, -1, -1):
                        if parts[i].startswith("icm/") or (
                            i > 0
                            and parts[i - 1].endswith("/lg")
                            or parts[i - 1].endswith("/rs")
                        ):
        ...
        if univariate_results:
            ...
            logger.info(
                f"Generated univariate predictions for {len(univariate_df)} markers"
            )
        else:
    ...
    def _univariate_prediction_simple(self, X_subject, feature_cols):
        ...
        logger.warning(
            "Results may not be accurate - retrain models with univariate support"
        )
...
def predict_icm_lg(markers_data, summary=None, config_params=None):
    ...
    if "clf_type" not in config_params:
        ...
        config_params["clf_select"] = {"gssvm": 20.0, "et-reduced": None}
...
def predict_icm_rs(markers_data, summary=None, config_params=None):
    ...
    if "reductions" not in config_params:
        config_params["reductions"] = [
            "icm/rs/egi256/trim_mean80",
            "icm/rs/egi256/std",
            "icm/rs/egi256gfp/trim_mean80",
            "icm/rs/egi256gfp/std",
        ]
