...
class LinearProber:
    ...
    def __init__(
        ...
        self.models = models or list(MODEL_LAYER_KEYS.keys())
    ...
    def collect_regression_data(self, model_name, layer_key):
        ...
        marker_names : list of str
        """
        embeddings = self._load_pooled_embeddings(model_name, layer_key)
...
python linear_probing.py \\
    ...
    parser.add_argument(
        ...
        help="Foundation models to probe.",
    )
    parser.add_argument(
        "--layers",
        default="all",
        help="Comma-separated layer keys or 'all'.",
    )
    parser.add_argument(
        "--skip-regression",
        action="store_true",
    ...
    for m in args.models:
        if m not in MODEL_LAYER_KEYS:
            parser.error(f"Unknown model {m!r}. Available: {list(MODEL_LAYER_KEYS)}")
