...
def plot_auc_comparison(all_results, output_path, classifiers):
    ...
    ax.set_title(
        "VS vs MCS classification after marker residualization\n(nested CV, last-layer embeddings)",
        fontsize=11,
    )
...
def run_model(
    ...
    """Run the full residualization + classification pipeline for one FM model.
...
def main():
    parser = argparse.ArgumentParser(
        description="Residualize FM embeddings w.r.t. EEG markers, then classify VS/MCS."
    )
    parser.add_argument(
        "--results-root",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "enabling direct comparison with other benchmark scripts."
        ),
    )
    parser.add_argument(
        "--save-splits-to",
        ...
        help=(
            ...
            "{output_dir}/cv_splits/residualization_crs_{timestamp}.json."
        ),
    )
    ...
    all_results = {}
    subjects_per_model = {}
    _splits_saved = precomputed_splits is not None  # only save when we generate new ones
    ...
    for model_name in models_to_run:
        ...
        if model_results is not None:
            ...
            # On the first successful run, lock in the splits for subsequent models
            if not _splits_saved and splits_used is not None:
                ...
                # Auto-save so the splits can be shared with other scripts
                save_path = args.save_splits_to
                if not save_path:
                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                    splits_dir = op.join(args.output_dir, "cv_splits")
                    save_path = op.join(
                        splits_dir, f"residualization_crs_{ts}.json"
                    )
