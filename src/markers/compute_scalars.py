...
def compute_scalars_from_h5(h5_file, output_file, logger):
    ...
    for i, feature_info in enumerate(features.values(), 1):
        ...
        try:
            ...
            if 'window_decoding' in display_name:
                skipped_features.append((display_name, "window_decoding marker (no epochs)"))
                continue
    ...
    for feature_info in features.values():
        ...
        if 'cnv_detailed_cnvintercept' in display_name or 'cnv_inception' in display_name:
        ...
        # 2. PE and WSMI: only theta values
        if 'pe_theta' in display_name or 'wsmi_theta' in display_name:
            ...
            if 'wsmi' in display_name:
                ...
                if len(data.shape) == 2 and data.shape[1] == n_pairs:
                    logger.info(f"  WSMI already shaped as ({data.shape[0]}, {n_pairs} pairs)")
                    ...
                    if len(data_flat) % n_pairs == 0:
                        ...
                        logger.warning(f"  Could not reshape WSMI: {len(data_flat)} not divisible by {n_pairs}")
                        continue
                else:
                ...
                logger.info(f"  Reconstructed full WSMI matrices: ({n_epochs_wsmi}, {n_channels}, {n_channels})")
        ...
        # Skip non-theta PE and WSMI
        if display_name.startswith('pe_') and 'theta' not in display_name:
            logger.info(f"  -> Skipping {display_name} (only theta PE used)")
            continue
        if display_name.startswith('wsmi_') and 'theta' not in display_name:
            logger.info(f"  -> Skipping {display_name} (only theta WSMI used)")
            continue
...
def main():
    ...
    parser.add_argument('--output_file', required=True, help='Path to output scalars file (.npz)')
