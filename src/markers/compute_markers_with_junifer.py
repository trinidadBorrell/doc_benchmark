...
def create_subject_yaml(template_yaml, fif_file, output_h5, logger):
    ...
    yaml_text = yaml_text.replace("uri: icm_complete_features.h5", f"uri: {output_h5}")

    # Update the datadir line - replace any existing datadir path
    yaml_text = re.sub(r'datadir: "[^"]*"', f'datadir: "{datadir}"', yaml_text)

    # Update the pattern line - use just the filename (relative pattern)
    yaml_text = re.sub(r'pattern: "[^"]*\.fif"', f'pattern: "{pattern}"', yaml_text)
...
def run_compute_data(
    h5_file, fif_file, subject_id, output_dir, skip_clustering, task, logger
):
    ...
    for idx, fif_file in enumerate(fif_files, 1):
        ...
        try:
            ...
            if not run_compute_data(
                ...
                logger.error(
                    f"❌ Failed to run compute_data.py for subject {subject_id}"
                )
                failed_subjects.append(subject_id)
                continue
            ...
            if not run_compute_scalars(h5_file, subject_id, subject_output_dir, logger):
                logger.error(
                    f"❌ Failed to run compute_scalars.py for subject {subject_id}"
                )
                failed_subjects.append(subject_id)
                continue
            ...
            if h5_file.exists():
                if args.keep_h5:
                    ...
                    h5_file.unlink()
                    logger.info(
                        f"H5 file deleted (use --keep-h5 to preserve): {h5_file}"
                    )
