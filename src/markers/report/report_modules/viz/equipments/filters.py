...
def _egi_filter(raw, params=None, summary=None, n_jobs=1):
    ...
    _filter_params = dict(
        method="iir", l_trans_bandwidth=0.1, iir_params=dict(ftype="butter", order=6)
    )
    filter_params = [
        dict(l_freq=hpass, h_freq=None, iir_params=dict(ftype="butter", order=6)),
        dict(l_freq=None, h_freq=lpass, iir_params=dict(ftype="butter", order=8)),
    ]
...
def _eximia_filter(raw, params=None, summary=None, n_jobs=1):
    ...
    picks = mne.pick_types(raw.info, eeg=True, meg=True, ecg=True, exclude=[])
    _filter_params = dict(
        method="iir", l_trans_bandwidth=0.1, iir_params=dict(ftype="butter", order=4)
    )
    filter_params = [
        dict(l_freq=hpass, h_freq=None, iir_params=dict(ftype="butter", order=4)),
        dict(l_freq=None, h_freq=lpass, iir_params=dict(ftype="butter", order=8)),
    ]
