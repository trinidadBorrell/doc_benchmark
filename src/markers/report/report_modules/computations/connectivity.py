...
def compute_connectivity_analysis_data(
...
def _compute_per_band_connectivity(
    epochs, report_data, output_dir="./tmp_computed_data"
):
        ...
        for band_name, marker_pattern in band_configs:
            ...
            if wsmi_data is not None:
                ...
                if wsmi_data.ndim == 3 and wsmi_data.shape[0] == 1:
                    ...
                    logger.info(
                        f"Squeezed WSMI {band_name} data to shape: {wsmi_data.shape}"
                    )
                ...
                if wsmi_data.ndim == 2 and wsmi_data.shape == (n_epochs, n_pairs):
                    ...
                    for epoch_idx in range(n_epochs):
                        ...
                        wsmi_matrices[epoch_idx] = (
                            wsmi_matrices[epoch_idx] + wsmi_matrices[epoch_idx].T
                        )

                    # Aggregate: median across channels_y, then mean across epochs
                    wsmi_median_per_ch = np.median(
                        wsmi_matrices, axis=2
                    )  # (epochs, channels)
                    wsmi_aggregated = np.mean(wsmi_median_per_ch, axis=0)  # (channels,)

                    logger.info(
                        f"Aggregated WSMI across {n_epochs} epochs: final shape {wsmi_aggregated.shape}"
                    )
