...
def compute_cnv_analysis(
    ...
    rois_chs = [get_roi(config=outlines, roi_name=roi_name) for roi_name in rois]
    ...
    _, pos, _, _, _, _, _ = _prepare_topomap_plot(epochs.info, "eeg", sphere=sphere)
    ...
    for roi_name, roi in zip(rois, rois_chs):
        ...
        if len(valid_roi) == 0:
            logger.warning(
                f"No valid channels for ROI {roi_name} (data has {n_channels} channels)"
            )
            continue
...
def compute_cluster_test(
    ...
    logger.info(
        f"Running spatio-temporal cluster test with {n_permutations} permutations..."
    )
    ...
    if n_clusters == 0:
        logger.info(f"No significant cluster. Plotting lowest p-value ({p_values[0]}).")
        n_clusters = 1
    else:
