...
def get_scalp_roi_indices(n_channels=256):
    ...
    if n_channels == 256:
        ...
        logger.warning(
            f"Unknown montage with {n_channels} channels, using all channels"
        )
    ...
    for step in reduction_config:
        ...
        if picks_config and axis_name in picks_config:
            ...
            if picks is not None:
                ...
                result = np.take(
                    result, picks, axis=0
                )  # Simplified - adjust axis as needed
...
def get_lg_reduction_config(reduction_type, marker_type=None):
    ...
    config = {"channels_fun": channels_fun, "epochs_fun": epochs_fun, "picks": picks}
...
def reduce_to_scalar(
    data, n_epochs, n_channels, marker_type, reduction_type, is_connectivity=False
):
    """
