...
def monitor_htcondor_jobs(cluster_ids, job_names, logger, check_interval=30):
    ...
    while True:
        ...
        logger.info(f"Progress: {completed_count} completed, {failed_count} failed, {running_count} running")
...
def monitor_multiple_dags_old(dag_files, logger, check_interval=30):
    ...
    while True:
        ...
        logger.info(f"Progress: {completed} completed, {failed} failed, {running} running")
...
def process_individual_result(h5_file, item_info, output_base_dir, skip_clustering, task, logger):
    ...
    logger.info(f"✅ compute_data.py completed")
    ...
    logger.info(f"✅ compute_scalars.py completed")
    return True
        ...
        try:
            ...
            for item in batch_items:
                ...
                yaml_file = create_individual_yaml(
                    template_yaml, item, h5_file, 
                    batch_yaml_dir, logger
                )
            ...
            logger.info(f"✅ Batch {batch_idx + 1}: {batch_success_count}/{len(batch_items)} completed successfully")
